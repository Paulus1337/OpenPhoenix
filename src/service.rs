...
mod tests {
    ...
    fn unit_pins_home_and_exec() {
        ...
        assert!(u.contains("RestartPreventExitStatus=2"));
        ...
        assert!(
            u.contains("TimeoutStopSec=35"),
            "graceful shutdown takes up to 30s; systemd must wait longer than that before SIGKILL"
        );
    }
