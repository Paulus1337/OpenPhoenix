    ...
    match ev["op"].as_u64() {
        ...
        Some(0) => match ev["t"].as_str() {
            ...
            Some("MESSAGE_CREATE") => {
                ...
                let author = d["author"]["id"].as_str().unwrap_or("").to_string();
...
impl Discord {
    ...
    pub fn new(cfg: &Config) -> Result<Discord, String> {
        ...
        Ok(Discord {
            ...
            allowed: cfg.discord_allowed.iter().cloned().collect(),
        })
    }
    ...
    fn api_post(&self, path: &str, body: &Value) -> Result<Value, String> {
        for attempt in 0..2 {
            ...
            match resp {
                ...
                Err(ureq::Error::Status(429, r)) if attempt == 0 => {
                    let wait = r
                        .header("Retry-After")
                        .and_then(|v| v.parse::<f64>().ok())
                        .unwrap_or(2.0);
                    std::thread::sleep(Duration::from_millis((wait * 1000.0) as u64 + 100));
                }
                Err(e) => return Err(self.scrub(&e.to_string())),
            }
        }
        Err("discord: rate limited twice, giving up".into())
    }
    ...
    pub fn serve(&self, handler: &mut dyn FnMut(&str, &str) -> String) {
        ...
        println!(
            "phoenix: serving discord ({} allowed channel(s))",
            self.allowed.len()
        );
        loop {
            let resuming = session.is_some();
            match self.run_socket(handler, &mut session, &mut seq) {
                Ok(keep_session) => {
                    backoff = 1;
                    if !keep_session {
                        session = None;
                ...
                Err(e) => {
                    println!("discord gateway error: {}, reconnecting", self.scrub(&e));
                    if !resuming {
                        session = None;
                        seq = None;
                    }
            ...
            match classify(&ev, seq) {
                ...
                Action::Ready(sid, resume_url) => {
                    *session = Some((sid, resume_url));
                    println!("phoenix: discord gateway ready");
                }
                Action::Message(channel, _author, content) => {
                    if !self.allowed.contains(&channel) {
                        continue;
                    }
                    let reply = handler(&channel, &content);
                    if !reply.is_empty() {
                        if let Err(e) = self.send(&channel, &reply) {
                            println!("discord send error: {}", self.scrub(&e));
                        }
                    }
                }
                Action::None => {}
