...
#[cfg(unix)]
const RAW_ARGS: [&str; 7] = ["-icanon", "-echo", "-isig", "min", "1", "time", "0"];
...
impl RawMode {
    ...
    fn block_mode(&self) {
        let _ = stty_on(&self.tty, &RAW_ARGS);
    }
...
#[cfg(unix)]
pub const INTERRUPT_EXIT: i32 = 130;
...
mod tests {
    ...
    #[test]
    #[cfg(unix)]
    fn raw_mode_disables_the_kernel_interrupt_so_the_tty_is_always_restored() {
        assert!(
            RAW_ARGS.contains(&"-isig"),
            "without -isig the kernel raises SIGINT on ^C, the default handler kills us before Drop runs, and the user's terminal is left with echo off"
        );
        assert!(RAW_ARGS.contains(&"-echo"));
        assert!(RAW_ARGS.contains(&"-icanon"));
    }

    #[test]
    #[cfg(unix)]
    fn an_interrupted_menu_exits_with_the_status_a_shell_expects() {
        assert_eq!(INTERRUPT_EXIT, 130);
    }
