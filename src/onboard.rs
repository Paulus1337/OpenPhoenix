...
PHOENIX_API_KEY later): "
    ...
    let stash_result = if carried && !keys.is_empty() {
        Some(crate::secrets::stash_provider_keys(&keys))
    } else if !api_key.is_empty() {
        Some(crate::secrets::stash_provider_keys(&[(
            provider.clone(),
            vec![api_key.clone()],
        )]))
    } else {
    ...
    let stashed = matches!(stash_result, Some(Ok(_)));
    if let Some(Err(e)) = &stash_result {
        let _ = writeln!(
            w,
            "could not encrypt the keys into the secret store ({e}); \
add them later with `phoenix secret set NAME`."
        );
    }
    let has_key = match &stash_result {
        Some(Err(_)) => {
            provider == "ollama" || (!provider.is_empty() && detected_key_var(&provider).is_some())
        }
        _ => has_key,
    };
    ...
    let stored = stashed;
    if stashed && carried && !keys.is_empty() {
        let summary: Vec<String> = keys
            .iter()
            .map(|(p, k)| format!("{p} ({})", k.len()))
            .collect();
        let _ = writeln!(
            w,
            "API keys encrypted into {}/secrets.enc (AES-256-GCM): {}. The unlock key \
PHOENIX_SECRET_KEY lives in {}/env (mode 600).",
            crate::config::home().display(),
            summary.join(", "),
            crate::config::home().display()
        );
    } else if stashed && !api_key.is_empty() {
        let _ = writeln!(
            w,
            "API key encrypted into the secret store (AES-256-GCM); the unlock key \
PHOENIX_SECRET_KEY lives in {}/env (mode 600).",
            crate::config::home().display()
        );
    }
...
network. Pick a stronger one (12+ characters), or Enter to keep the UI off."
    ...
    if !plan.api_key.is_empty() {
        match crate::secrets::stash_provider_keys(&[(
            plan.provider.clone(),
            vec![plan.api_key.clone()],
        )]) {
            Ok(_) => {
                plan.api_key = String::new();
                let _ = writeln!(
                    w,
                    "API key encrypted into the secret store (AES-256-GCM); the unlock key \
PHOENIX_SECRET_KEY lives in {}/env (mode 600).",
            ...
            Err(e) => {
                ...
                let _ = writeln!(
                    w,
                    "could not encrypt the key into the secret store ({e}); \
add it later with `phoenix secret set NAME` or export it as an env var."
                );
            }
        }
    }
...
y\n9911\n1\nhttp-tok\ny\nadmin\nT3rn-Owl-Rises\n1,3\ny\n"
    ...
    fn isolate_state(dir: &std::path::Path) -> StateGuard {
        let g = StateGuard {
            old_state: std::env::var("PHOENIX_STATE_DIR").ok(),
            old_key: std::env::var(crate::secrets::KEY_VAR).ok(),
        };
    ...
    impl Drop for StateGuard {
        fn drop(&mut self) {
            match &self.old_state {
                Some(v) => std::env::set_var("PHOENIX_STATE_DIR", v),
                None => std::env::remove_var("PHOENIX_STATE_DIR"),
            }
            match &self.old_key {
                Some(v) => std::env::set_var(crate::secrets::KEY_VAR, v),
                None => std::env::remove_var(crate::secrets::KEY_VAR),
            }
        }
    }
    ...
    fn mig_dir(name: &str) -> (std::path::PathBuf, std::path::PathBuf) {
        ...
        (dir.join("config.toml"), dir.join("openclaw.json"))
    }
    ...
    fn wizard_migration_carries_gateway_key() {
        ...
        .unwrap();
        let _l = state_lock();
        let state = path.parent().unwrap().join("state");
        let _g = isolate_state(&state);
        let mut input = Cursor::new(b"\n\n\n\n".to_vec());
        let mut out = Vec::new();
        let ok = first_run(Some(&src), &path, false, &mut input, &mut out).unwrap();
        assert!(ok);
        ...
        assert!(
            ...
            "the key must never land in config.toml: {raw}"
        );
        let store = crate::secrets::Store::at(&crate::secrets::Store::default_path());
        assert_eq!(
            store.get("OPENROUTER_API_KEY").as_deref(),
            Some("carried-or-key"),
            "the carried key must live in the encrypted store"
        );
        let text = String::from_utf8(out).unwrap();
        assert!(text.contains("Carry it over"));
        assert!(text.contains("encrypted into the secret store"), "{text}");
    }
    ...
    fn wizard_migration_carries_all_keys_and_picks_models() {
        ...
        assert_eq!(store.get("OPENROUTER_API_KEY").as_deref(), Some("or-zero"));
        assert_eq!(store.get("OPENROUTER_API_KEY_2").as_deref(), Some("or-one"));
        assert_eq!(store.get("NVIDIA_API_KEY").as_deref(), Some("nv-key"));
        let text = String::from_utf8(out).unwrap();
        assert!(text.contains("anthropic (1), nvidia (1), openrouter (2)"));
        assert!(text.contains("encrypted into"), "{text}");
    }
}
