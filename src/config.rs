...
# [secrets]                  # pull secrets from a keyvault CLI at startup (env vars win first)
# command = "vault kv get -field={name} secret/phoenix"   # {name} = ANTHROPIC_API_KEY etc.
...
pub fn parse(raw: &str) -> Result<Config, String> {
    ...
    let sec_vault = tbl(&root, "secrets");
...
0.0.0.0 for every IPv4 interface, or :: for every interface"
    ...
    let cfg = Config {
        ...
        secrets_command: get_str(sec_vault, "command", ""),
...
pub fn load(path: Option<&Path>) -> Result<Config, String> {
    ...
    if let Some(v) =
        crate::secrets::resolve_chain(&cfg.secrets_command, "PHOENIX_API_KEY", "api_key")
    {
        cfg.api_key = v;
    }
    ...
    if cfg.api_key.is_empty() {
        for var in provider_key_vars(&cfg.provider) {
            if let Some(v) = crate::secrets::resolve_chain(&cfg.secrets_command, var, var) {
                cfg.api_key = v;
    ...
    if let Some(v) = crate::secrets::resolve_chain(
        &cfg.secrets_command,
    ...
    if let Some(v) =
        crate::secrets::resolve_chain(&cfg.secrets_command, "PHOENIX_HTTP_TOKEN", "http_token")
    {
        cfg.http_token = v;
    }
    if let Some(v) = crate::secrets::resolve_chain(
...
mod json_schema_tests {
    ...
    fn types_are_declared_not_all_strings() {
        ...
        assert_eq!(p["agent"]["properties"]["compact_after"]["type"], "integer");
