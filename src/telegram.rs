...
pub fn progress_line(name: &str, args: &serde_json::Value) -> String {
    let detail = [
        "command", "task", "query", "url", "path", "prompt", "id", "note",
    ]
    .iter()
    .find_map(|k| args.get(*k).and_then(serde_json::Value::as_str))
    .unwrap_or("");
    let detail = detail.trim().replace('\n', " ");
    let short: String = detail.chars().take(64).collect();
    let ellipsis = if detail.chars().count() > 64 {
        "…"
    } else {
        ""
    };
...
impl Telegram {
    ...
    fn typing(&self, chat_id: &str, thread: Option<i64>) {
        let tid = thread.map(|t| t.to_string());
    ...
    pub fn abort_requested(&self, offset: i64) -> bool {
        let offset_s = offset.to_string();
        let Ok((200, v)) = self.call_status(
            "getUpdates",
            &[
        ...
        v["result"]
            ...
            .filter(|u| {
                u["message"]["chat"]["id"]
                    .as_i64()
                    .map(|id| self.allowed.allows(&id.to_string()))
                    .unwrap_or(false)
            })
            .filter_map(|u| u["message"]["text"].as_str())
            .any(|t| {
                let head = t.split_whitespace().next().unwrap_or("");
                let head = head.split('@').next().unwrap_or(head).to_lowercase();
                head == "/stop" || crate::is_abort_request(t.trim())
            })
    }
    ...
    fn clear_webhook(&self) {
        match self.call_status("deleteWebhook", &[], CTL_TIMEOUT_SECS) {
            ...
            Ok((code, body)) => {
                ...
                println!("telegram: deleteWebhook returned {code} ({msg}), continuing");
            }
            Err(e) => {
                let msg = crate::security::one_line(&redact(&e), 200);
                println!("telegram: deleteWebhook failed ({msg}), continuing");
            }
        }
    }
    ...
    fn startup_probe(&self) -> Result<String, String> {
        ...
        loop {
            ...
            match self.call_status("getMe", &[], CTL_TIMEOUT_SECS) {
                ...
                Ok((code, _)) => {
                    println!("telegram: getMe returned {code}, retrying in {delay}s");
                }
                Err(e) => {
                    let msg = crate::security::one_line(&redact(&e), 200);
                    println!("telegram: getMe failed ({msg}), retrying in {delay}s");
                }
            }
            if !self.pause(delay) {
                return Err("stop requested during telegram startup".into());
            ...
            let updates = match self.call_status(
                ...
                Ok((code, body)) => {
                    ...
                    match error_code {
                        ...
                        409 => {
                            println!(
                                "telegram: getUpdates conflict ({msg}), clearing webhook and retrying"
                            );
                            self.clear_webhook();
                            if !self.pause(2) {
                                return Ok(());
                        ...
                        429 => {
                            let wait = body["parameters"]["retry_after"]
                                ...
                                .unwrap_or(5)
                                .clamp(1, 300);
                            println!("telegram: rate limited, polling again in {wait}s");
                            if !self.pause(wait) {
                                return Ok(());
                            }
                        }
                        _ => {
                            println!("telegram poll error {error_code} ({msg}), retrying");
                            if !self.pause(5) {
                                return Ok(());
                            }
                        }
...
Content-Length: {}\r\n\r\n{payload}",
    ...
    #[test]
    fn markdown_escapes_and_refuses_unsafe_links() {
    ...
    #[test]
    fn progress_line_shows_tool_and_detail() {
        let l = progress_line("shell", &serde_json::json!({"command": "cargo test"}));
        assert_eq!(l, "🔥 working…\n🧰 shell: cargo test");
        let l = progress_line("web_search", &serde_json::json!({"query": "rust sha1"}));
        assert!(l.contains("🧰 web_search: rust sha1"));
        let l = progress_line("canvas_hide", &serde_json::json!({}));
        assert_eq!(l, "🔥 working…\n🧰 canvas_hide");
    }

    #[test]
    fn progress_line_truncates_and_flattens() {
        let long = "x".repeat(100);
        let l = progress_line("shell", &serde_json::json!({"command": long}));
        assert!(l.ends_with('…'), "{l}");
        assert!(l.chars().count() < 90, "{l}");
        let l = progress_line("shell", &serde_json::json!({"command": "a\nb"}));
        assert!(l.contains("a b"), "{l}");
    }
