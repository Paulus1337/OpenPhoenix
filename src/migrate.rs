...
pub fn collect_keys(gateway_dir: &std::path::Path) -> Vec<(String, Vec<String>)> {
    let raw = match std::fs::read_to_string(gateway_dir.join("secrets.json")) {
        Ok(r) => r,
        Err(_) => return Vec::new(),
    };
    let secrets: Value = match serde_json::from_str(&raw) {
        Ok(v) => v,
        Err(_) => return Vec::new(),
    };
    ...
    if let Ok(envf) = std::fs::read_to_string(gateway_dir.join("gateway.systemd.env")) {
        for line in envf.lines() {
            if let Some((k, val)) = line.trim().split_once('=') {
                let val = val.trim();
                if k.trim() == "CLAUDE_CODE_OAUTH_TOKEN" && !val.is_empty() {
                    entries.insert(
                        0,
