...
pub fn check(cfg: &Config, cfg_path: &Path, raw: &str, memory_path: &Path) -> Vec<Finding> {
    ...
    let (_, inline) = crate::config::strip_inline_secrets(raw);
    if !inline.is_empty() {
        ...
        out.push(f(
            "warn",
            format!(
                "plain text secrets in the config: {}; encrypt them with `phoenix secret import`",
                names.join(", ")
            ),
        ));
    } else {
        out.push(f("ok", "no plain text secrets in the config file"));
    }
...
mod tests {
    ...
    fn warns_on_secrets_in_file() {
        ...
        assert!(
            warns
                .iter()
                .any(|x| x.msg.contains("PHOENIX_API_KEY")
                    && x.msg.contains("PHOENIX_TELEGRAM_TOKEN")),
            "every plain text secret is named, not just the api key: {warns:?}"
        );
        assert!(
            warns.iter().any(|x| x.msg.contains("secret import")),
            "the warning must say how to fix it: {warns:?}"
        );
    }
