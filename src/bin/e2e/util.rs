...
impl Cx {
    pub fn detect() -> Result<Cx, String> {
        ...
        let key = PathBuf::from(
            std::env::var("E2E_KEY").unwrap_or_else(|_| "/e2e/signing.key".to_string()),
        );
        if !phx.is_file() {
...
pub fn sign_detached(key: &Path, data: &[u8]) -> Result<Vec<u8>, String> {
    let der = fs::read(key).map_err(|e| format!("read key {}: {e}", key.display()))?;
    let pair = ring::signature::Ed25519KeyPair::from_pkcs8(&der)
        .map_err(|_| "signing key is not valid pkcs8".to_string())?;
