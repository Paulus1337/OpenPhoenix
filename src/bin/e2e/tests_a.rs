...
pub fn t01_binary_smoke(cx: &Cx) -> T {
    ...
    let listed = r
        ...
        .unwrap_or(0);
    t.check("commands lists a plausible surface", listed >= 60);
    ...
    t.check("commands --json is valid JSON", doc.is_some());
    let counted = doc
        .as_ref()
        .and_then(|d| d.get("commands")?.as_array().map(Vec::len))
        .unwrap_or(0);
    t.check("the text and json surfaces agree", counted == listed);
...
pub fn t05_commands_reject_unknown(cx: &Cx) -> T {
    ...
    let r = util::phx(cx, &home, &["voicecall"]);
    t.check("not-built voicecall exits 2", r.rc == 2);
    t.check(
        "not-built voicecall names its reason",
        r.all().contains("not built here"),
    );
    let r = util::phx(cx, &home, &["worker"]);
    t.check(
        "not-built worker names its reason",
        r.all().contains("not built here"),
    );
    let r = util::phx(cx, &home, &["acp", "--help"]);
    t.check(
        "acp is built here and does not claim otherwise",
        !r.all().contains("not built here"),
    );
    let r = util::phx(cx, &home, &["commands", "--json"]);
...
pub fn t07_serve_startup(cx: &Cx) -> T {
    ...
    t.check(
        "http without token names the problem",
        r.all().contains("no bearer credential"),
    );
    ...
    match sv {
        Ok(mut sv) => {
            ...
            t.check(
                "startup is a structured info event",
                log.lines().any(|line| {
            ...
            t.check(
                "http listener is a structured info event",
                log.lines().any(|line| {
                    serde_json::from_str::<serde_json::Value>(line).is_ok_and(|event| {
                        event["level"] == "info"
                            && event["module"] == "http"
                            && event["msg"]
                                .as_str()
                                .is_some_and(|msg| msg.contains("API listening"))
                    })
                }),
            );
            let r = util::phx(cx, &home, &["serve"]);
