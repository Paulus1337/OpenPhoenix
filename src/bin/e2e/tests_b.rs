...
pub fn t13_migration(cx: &Cx) -> T {
    ...
    let claw = home.join(".openclaw");
    let _ = fs::create_dir_all(claw.join("workspace").join("memory"));
    let cfgj = claw.join("openclaw.json");
...
pub fn t14_session_quarantine(cx: &Cx) -> T {
    ...
    t.check(
        ...
        util::file_has(&sv.log, "kept a copy"),
    );
    t.check(
...
pub fn t16_update(cx: &Cx) -> T {
    ...
    if !cx.key.is_file() {
        t.bad(&format!("no signing key staged at {}", cx.key.display()));
    }
    let asset = rdir.join("phoenix-linux-x86_64");
    let sig = rdir.join("phoenix-linux-x86_64.sig");
    ...
    let sign_to = |t: &mut T, data: &[u8], sig_path: &std::path::Path| match util::sign_detached(
        &cx.key, data,
    ) {
        Ok(s) => {
            let _ = fs::write(sig_path, s);
        }
        Err(e) => t.bad(&format!("signing failed: {e}")),
    };
    let _ = fs::write(&asset, &new_bin);
    sign_to(&mut t, &new_bin, &sig);
    let _ = util::write_s(
        &sums,
    ...
    let _ = fs::write(&asset, &new_bin);
    let _ = util::write_s(&sums, &format!("{good_sum}  phoenix-linux-x86_64\n"));
    let _ = fs::write(&sig, b"garbage-signature-bytes");
    let r = util::run_in(&home, &ut, &["update"], &envs, None, 120_000);
    t.check(&format!("bad signature rejected (rc={})", r.rc), r.rc != 0);
    t.check(
        "rejection names the signature",
        r.all().contains("signature"),
    );
    let after = util::sha256_hex(&fs::read(&ut).unwrap_or_default());
    t.check("binary untouched after bad signature", before == after);
    let _ = fs::remove_file(&sig);
    let r = util::run_in(&home, &ut, &["update"], &envs, None, 120_000);
    t.check(
        &format!("unsigned release refused (rc={})", r.rc),
        r.rc != 0,
    );
    t.check(
        "refusal names the missing sig",
        r.all().contains("unsigned"),
    );
    let _ = fs::write(&asset, &new_bin);
    sign_to(&mut t, &new_bin, &sig);
    let _ = util::write_s(&sums, &format!("{good_sum}  phoenix-linux-x86_64\n"));
    let r = util::run_in(&home, &ut, &["update"], &envs, None, 120_000);
    if r.rc == 0 {
        t.ok("valid signed update succeeds");
    } else {
        let head: String = r.all().lines().take(2).collect::<Vec<_>>().join(" | ");
...
pub fn t18_doctor(cx: &Cx) -> T {
    ...
    t.check("doctor on a fresh nest exits 0", r.rc == 0);
    t.check(
        "doctor reports ok findings",
        util::line_starts(&r.out, "ok"),
    );
    t.check(
        "doctor finds no failures on a fresh nest",
        !util::line_starts(&r.out, "FAIL"),
    );
    let r = util::phx(cx, &home, &["doctor", "--json"]);
    let shaped = serde_json::from_str::<serde_json::Value>(&r.out)
        .ok()
        .map(|d| d.get("findings").map(|f| f.is_array()).unwrap_or(false))
        .unwrap_or(false);
    t.check("doctor --json is valid and shaped", shaped);
    let Some(home) = home_or(&mut t) else {
        return t;
    };
    let port = util::free_port();
...
pub fn t19_telegram_poll(cx: &Cx) -> T {
    ...
    t.check(
        "serve banner announces telegram",
        util::file_has(&sv.log, "serving telegram as @e2ebot"),
    );
    let _ = sv.term();
    let Some(home) = home_or(&mut t) else {
        return t;
