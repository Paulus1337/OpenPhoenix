...
pub fn provider(opts: ProviderOpts) -> Result<Mock, String> {
    ...
    let handler = move |req: &Req| -> Resp {
        ...
        if !opts.down_model.is_empty() && model == opts.down_model {
            return Resp::json(401, &json!({"error": {"message": "mock model offline"}}));
        }
        let marker = if opts.converge {
            " [[COLAB_CONVERGED]]"
        } else {
            ""
        };
        let text = format!("mock reply model={model}{marker}");
...
pub fn release(dir: PathBuf) -> Result<Mock, String> {
    ...
    let handler = move |req: &Req| -> Resp {
        ...
        if req.path.contains("releases/latest") {
            ...
            let names = ["SHA256SUMS", "phoenix-linux-x86_64"];
