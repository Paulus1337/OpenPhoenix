...
const EMPTY_REPLY_RETRIES: u32 = 2;
...
return its final answer. The child has its own tools and wall-clock \
budget but no access to this conversation. Use for isolated research or side work.",
        "parameters": {"type": "object",
...
answer now with what you have"
            ...
            let mut result = if self.stream_stdout {
            ...
            for attempt in 1..=BUSY_RETRIES {
                ...
                eprintln!(
                    "model busy ({}); retry {attempt} of {BUSY_RETRIES} in {attempt}s",
            ...
            if reply.tool_calls.is_empty() {
                if reply.text.trim().is_empty() {
                    ...
                    if empty_replies <= EMPTY_REPLY_RETRIES {
                        eprintln!(
                            "empty model reply; nudge {empty_replies} of {EMPTY_REPLY_RETRIES}"
                        );
                        self.history.push(Msg::User {
                            content: "[your previous reply was empty; answer now with \
visible text]"
                                .to_string(),
                            images: Vec::new(),
                        });
                    ...
                    reply.text = format!(
                        "the model sent back nothing {} times in a row; try again, or \
switch models with /model",
            ...
            for tc in reply.tool_calls.clone() {
                ...
                let warning = match self.loops.detect(&tc.name, &tc.args) {
                    crate::loop_detect::Detection::Warn(msg) => Some(msg),
                    crate::loop_detect::Detection::Ok => None,
                };
        ...
        if final_text.is_empty() {
            "hmm, that reply had no visible text. Ask again, or switch models with /model".into()
        } else {
            final_text
        }
    }
...
context: ~{used} / {window} tokens ({pct}%)\nturn limit: none (the model decides)\n\
...
mod tests {
    ...
    impl ChatBackend for LoopProvider {
        fn chat(
            ...
            let warned = history.iter().any(|m| match m {
                Msg::Tool { content, .. } => content.contains("loop warning"),
                _ => false,
            });
            if warned {
                return Ok(Reply::text_only(
                    "stopping: the loop warning says this is going nowhere",
                ));
            }
            Ok(Reply {
                text: String::new(),
    ...
    #[test]
    fn degenerate_loops_warn_the_model_which_decides_to_stop() {
        let cfg = make_cfg("session");
        let mut agent = build(&cfg, Box::new(LoopProvider));
        let out = agent.run("loop forever");
        assert_eq!(
            out, "stopping: the loop warning says this is going nowhere",
            "the warning must reach the model and the model must make the call"
        );
        let warned = agent.history.iter().any(|m| match m {
            Msg::Tool { content, .. } => content.contains("loop warning"),
            _ => false,
        });
        assert!(warned, "the loop warning must land in a tool result");
    }
    ...
    impl ChatBackend for EmptyThenTextProvider {
        fn chat(
            &mut self,
            _cfg: &Config,
            _system: &str,
            ...
            self.calls += 1;
            let text = if self.calls <= 2 { "" } else { "real answer" };
            Ok(Reply {
                text: text.to_string(),
                tool_calls: Vec::new(),
                usage: Usage::default(),
    ...
    impl ChatBackend for AlwaysEmptyProvider {
        fn chat(
            &mut self,
            _cfg: &Config,
            _system: &str,
    ...
    #[test]
    fn empty_replies_are_retried_until_text_arrives() {
        let cfg = make_cfg("session");
        let mut agent = build(&cfg, Box::new(EmptyThenTextProvider { calls: 0 }));
        let out = agent.run("say something");
        assert_eq!(out, "real answer");
    }

    #[test]
    fn persistently_empty_replies_get_an_honest_explanation() {
        let cfg = make_cfg("session");
        let mut agent = build(&cfg, Box::new(AlwaysEmptyProvider));
        let out = agent.run("say something");
        assert!(
            out.contains("nothing") && out.contains("/model"),
            "the user must get an explanation, not a placeholder: {out}"
        );
        assert_ne!(out, "(empty reply)");
    }
...
Content-Length: {}\r\nConnection: close\r\n\r\n{}",
    ...
    #[test]
    fn stuck_loops_warn_and_the_model_decides() {
        struct Repeater {
        ...
        impl ChatBackend for Repeater {
            fn chat(
                ...
                let warned = h.iter().any(|m| match m {
                    Msg::Tool { content, .. } => content.contains("loop warning"),
                    _ => false,
                });
                if warned {
                    return Ok(Reply::text_only("wrapping up: no progress here"));
                }
        ...
        let out = agent.run("go");
        assert_eq!(out, "wrapping up: no progress here");
        let warned = agent.history.iter().any(|m| match m {
            Msg::Tool { content, .. } => content.contains("loop warning"),
            _ => false,
        });
        assert!(
            warned,
            "a warning must be delivered so the model can decide"
        );
    }
