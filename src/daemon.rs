...
pub fn set_service_mode(enabled: bool) {
    SERVICE_MODE.store(enabled, Ordering::SeqCst);
}

#[cfg(test)]
pub fn reset_signal_state() {
    SERVICE_MODE.store(false, Ordering::SeqCst);
...
pub fn install_handlers() -> Result<(), String> {
    ...
    ctrlc::set_handler(|| {
        if INTERRUPT_ARMED.load(Ordering::SeqCst) && !SERVICE_MODE.load(Ordering::SeqCst) {
            INTERRUPT_PENDING.store(true, Ordering::SeqCst);
        } else {
            STOPPING.store(true, Ordering::SeqCst);
        }
    })
    .map_err(|error| format!("signal handler: {error}"))
}
...
pub fn spawn_shutdown_watchdog(grace: std::time::Duration) {
    std::thread::spawn(move || loop {
        if stopping() {
            ...
            crate::log::error(
                "daemon",
                format!("a turn was still running {secs}s after stop was requested; forcing exit"),
            );
...
mod tests {
    ...
    #[test]
    fn service_mode_is_explicit_and_resettable() {
        reset_signal_state();
        assert!(!SERVICE_MODE.load(Ordering::SeqCst));
