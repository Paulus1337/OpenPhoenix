...
pub fn classify(ev: &Value) -> Option<(String, String, String)> {
    ...
    let (dm, edited) = if envelope["dataMessage"].is_object() {
        (&envelope["dataMessage"], false)
    } else if envelope["editMessage"]["dataMessage"].is_object() {
        (&envelope["editMessage"]["dataMessage"], true)
    } else {
        return None;
    };
    if dm["reaction"].is_object() {
        return None;
    }
    ...
    let label = envelope["sourceName"]
        .as_str()
        .filter(|s| !s.trim().is_empty())
        .unwrap_or(sender)
        .to_string();
    if edited {
        return Some((sender.to_string(), label, format!("(edited) {text}")));
    }
    Some((sender.to_string(), label, text.to_string()))
}
...
impl Signal {
    ...
    pub fn new(cfg: &Config) -> Result<Signal, String> {
        ...
        Ok(Signal {
            ...
            allowed: crate::allowlist::Allowlist::new(&cfg.signal_allowed),
    ...
    pub fn typing_payload(recipient: &str, stop: bool) -> Value {
        let mut payload = json!({ "recipient": [recipient] });
        if stop {
            payload["stop"] = json!(true);
        }
        payload
    }

    pub fn typing(&self, recipient: &str, stop: bool) {
        let _ = self.rpc("sendTyping", Self::typing_payload(recipient, stop));
    }

    pub fn working(&self, recipient: &str) -> crate::working::Working {
        let start_recipient = recipient.to_string();
        let stop_recipient = start_recipient.clone();
        let start_signal = self.clone();
        let stop_signal = self.clone();
        crate::working::Working::native(
            crate::working::NATIVE_INTERVAL,
            move || start_signal.typing(&start_recipient, false),
            move || stop_signal.typing(&stop_recipient, true),
        )
    }
    ...
    pub fn serve(&self, handler: &mut dyn FnMut(&str, &str) -> String) {
        ...
        crate::log::info_with(
            "signal",
            format!("serving with {} allowed number(s)", self.allowed.len()),
            &crate::log::Fields::default().channel("signal"),
        );
        loop {
            match self.run_daemon(handler, &mut last_seen) {
                Ok(()) => backoff = 1,
                Err(e) => {
                    crate::log::warn_with(
                        "signal",
                        format!("daemon failed: {}; restarting", redact(&e)),
                        &crate::log::Fields::default().channel("signal"),
                    );
    ...
    fn run_daemon(
        ...
        let result = (|| {
            ...
            self.stream_events(&mut child, handler, last_seen)
        })();
        let _ = child.kill();
        let _ = child.wait();
        result
    ...
    fn stream_events(
        ...
        loop {
            ...
            let Some((sender, label, text)) = classify(&ev) else {
                continue;
            };
            ...
            let mut working = self.working(&sender);
...
mod tests {
    ...
    fn classify_plain_message() {
        ...
        assert_eq!(
            classify(&ev),
            Some((
                "+4915551234567".to_string(),
                "+4915551234567".to_string(),
                "hi phoenix".to_string()
            ))
        );
    }

    #[test]
    fn classify_carries_the_profile_name_when_present() {
        let ev: Value = serde_json::from_str(
            r#"{"envelope":{"sourceNumber":"+4915551234567","sourceName":"Paulus",
                "dataMessage":{"message":"hi","timestamp":1}},"account":"+490"}"#,
        )
        .unwrap();
        assert_eq!(
            classify(&ev),
    ...
    fn inbound_text_envelopes_chat_but_not_commands() {
        let out = inbound_text("Paulus", "hello", Some(90));
        assert!(out.starts_with("[Signal Paulus +2m "), "{out}");
        assert!(out.ends_with("] hello"), "{out}");
        assert_eq!(inbound_text("Paulus", "/approve 1", None), "/approve 1");
    }
    ...
    #[test]
    fn classify_edit_envelopes_deliver_the_edited_text() {
        let ev: Value = serde_json::from_str(
            r#"{"envelope":{"sourceNumber":"+4915551234567",
                "editMessage":{"targetSentTimestamp":1,
                "dataMessage":{"message":"corrected text","timestamp":2}}}}"#,
        )
        .unwrap();
        assert_eq!(
            classify(&ev),
            Some((
                "+4915551234567".to_string(),
                "+4915551234567".to_string(),
                "(edited) corrected text".to_string()
            ))
        );
    }

    #[test]
    fn classify_edited_reactions_stay_ignored() {
        let ev: Value = serde_json::from_str(
            r#"{"envelope":{"sourceNumber":"+49",
                "editMessage":{"dataMessage":{"message":"","reaction":{"emoji":"x"}}}}}"#,
        )
        .unwrap();
    ...
    #[test]
    fn typing_payload_starts_and_explicitly_stops() {
        assert_eq!(
            Signal::typing_payload("+491", false),
            json!({"recipient": ["+491"]})
        );
        assert_eq!(
            Signal::typing_payload("+491", true),
            json!({"recipient": ["+491"], "stop": true})
        );
    }
