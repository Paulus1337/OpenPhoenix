...
pub fn serve(port: u16, upstream: &str, capture: &Path) -> Result<(), String> {
    ...
    crate::log::info_with(
        "proxy",
        format!("listener ready on loopback port {actual}"),
        &crate::log::Fields::default().channel("proxy"),
    );
        ...
        match forward(upstream, &req) {
            Ok((status, headers, body)) => {
                ...
                crate::log::debug_with(
                    "proxy",
                    format!("request completed method={} status={status}", req.method),
            ...
            Err(e) => {
                ...
                crate::log::warn_with(
                    "proxy",
                    format!("upstream request failed method={}", req.method),
                    &crate::log::Fields::default()
                        .channel("proxy")
                        .duration_ms(crate::log::millis(started.elapsed())),
                );
