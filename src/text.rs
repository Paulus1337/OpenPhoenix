...
pub const BOX_ACCENT: &str = "\x1b[38;5;208;1m";
pub const BOX_DIM: &str = "\x1b[2m";
pub const BOX_OFF: &str = "\x1b[0m";
pub const BOX_OK: &str = "\x1b[32;1m";
pub const BOX_WARN: &str = "\x1b[33;1m";
pub const BOX_FAIL: &str = "\x1b[31;1m";

pub fn status_tag(level: &str, color: bool) -> String {
    let (word, code) = match level {
        "fail" | "FAIL" => ("FAIL", BOX_FAIL),
        "warn" => ("warn", BOX_WARN),
        _ => ("ok", BOX_OK),
    };
    if color {
        format!("{code}{word}{BOX_OFF}")
    } else {
        word.to_string()
    }
}

pub fn draw_box(title: &str, lines: &[String], boxed: bool) -> String {
    if !boxed {
    ...
    let content_width = lines
        ...
        .max()
        .unwrap_or(0)
        .max(visible_len(title) + 2)
        .max(20);
    let accent = BOX_ACCENT;
    let off = BOX_OFF;
    let mut out = String::new();
    let title_bar_len = content_width
        .saturating_sub(visible_len(title))
        .saturating_sub(1);
    out.push_str(&format!(
        "{accent}\u{250c}\u{2500} {title} {}\u{2510}{off}\n",
        "\u{2500}".repeat(title_bar_len)
    ));
    for line in lines {
        let pad = content_width.saturating_sub(visible_len(line));
        out.push_str(&format!(
            "{accent}\u{2502}{off} {line}{} {accent}\u{2502}{off}\n",
            " ".repeat(pad)
        ));
    }
    out.push_str(&format!(
        "{accent}\u{2514}{}\u{2518}{off}\n",
...
mod tests {
    ...
    #[test]
    fn middle_truncate_keeps_head_and_tail_within_the_budget() {
        assert_eq!(middle_truncate("short", 10), "short");
        let out = middle_truncate("deepseek-ai/deepseek-v4-flash-0731", 20);
        assert_eq!(out.chars().count(), 20);
        assert!(out.contains('\u{2026}'), "{out}");
        assert!(out.starts_with("deepseek"), "{out}");
        assert!(out.ends_with("0731"), "{out}");
        assert_eq!(middle_truncate("abcdefghij", 10), "abcdefghij");
        assert_eq!(middle_truncate("abcdefghijk", 1), "\u{2026}");
    }
    ...
    fn comment_and_forbidden_offsets(src: &str) -> (Vec<usize>, Vec<usize>) {
        let b = src.as_bytes();
        let n = b.len();
        ...
        while i < n {
            ...
            if c == b'u' && !prev_ident && src[i..].starts_with(&["un", "safe"].concat()) {
                let after = i + 6;
                let boundary =
    ...
    fn no_source_file_contains_comments_outside_string_literals() {
        ...
        for (name, body) in rust_sources(&dir) {
            let (comments, _) = comment_and_forbidden_offsets(&body);
            for off in comments {
    ...
    #[test]
    fn no_source_file_contains_forbidden_code() {
        let dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src");
        let mut offenders: Vec<String> = Vec::new();
        for (name, body) in rust_sources(&dir) {
            let (_, forbidden) = comment_and_forbidden_offsets(&body);
        ...
        assert!(
            offenders.is_empty(),
            "forbidden Rust construct found in shipped source: {offenders:?}"
        );
    }
