...
Content-Security-Policy: default-src 'none'; script-src 'self' 'nonce-phoenix'; \
style-src 'self' 'nonce-phoenix'; connect-src 'self'; img-src 'self' data:; \
base-uri 'none'; form-action 'self'; frame-ancestors 'none'\r\n\
X-Content-Type-Options: nosniff\r\nX-Frame-Options: DENY\r\n\
Referrer-Policy: no-referrer\r\nCache-Control: no-store\r\n{robots}\
Content-Length: {}\r\nConnection: close\r\n\r\n{body}",
...
Content-Length: {}\r\n\r\n{body}",
    ...
    fn canvas_serves_document_and_version_with_creds() {
        ...
        assert!(resp.contains("'nonce-phoenix'"), "canvas CSP nonce missing");
        let resp = talk(
            addr,
