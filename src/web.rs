...
.locked-egg {
  width: 84px;
  height: 84px;
  filter: drop-shadow(0 0 12px rgba(255, 140, 46, 0.45));
...
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta name="color-scheme" content="dark light">
<title>OpenPhoenix - locked</title>
<link rel="icon" type="image/svg+xml" href="/favicon.svg">
<link rel="stylesheet" href="/style.css">
</head>
<body>
<header>
  ...
  <div class="title">
    <h1>OpenPhoenix</h1>
    <div class="tag">every door starts locked</div>
  </div>
</header>
<main class="locked">
  <img class="locked-egg" src="/egg.svg" alt="">
  <h2>This nest is locked</h2>
  <p>Sign in with the username and password set in <code>[http]</code> to reach
  the chat UI. Your browser should have already prompted for them; if it did
  not, reload this page.</p>
  <p class="dim">Running the server yourself? The credentials live in
  <code>http.username</code> and <code>http.password</code> in
  <code>config.toml</code>, or <code>PHOENIX_HTTP_PASS</code> in the
  environment.</p>
</main>
</body>
</html>
