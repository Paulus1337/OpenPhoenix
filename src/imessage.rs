...
impl RpcWriter {
    fn send(&self, target: &Value, text: &str) -> Result<(), String> {
        let mut params = target.clone();
        params["text"] = Value::from(text);
        let id = self.next_id.fetch_add(1, Ordering::Relaxed);
        let req = json!({
            "jsonrpc": "2.0", "id": id, "method": "send", "params": params
        });
        let mut stdin = self
            .stdin
            .lock()
            .map_err(|_| "imessage: writer lock poisoned".to_string())?;
        writeln!(stdin, "{req}").map_err(|e| format!("imessage: send: {e}"))?;
        stdin.flush().map_err(|e| format!("imessage: send: {e}"))
    }
}
...
impl IMessage {
    ...
    pub fn serve(&self, handler: &mut dyn FnMut(&str, &str) -> String) {
        ...
        crate::log::info_with(
            "imessage",
            format!("serving with {} allowed sender(s)", self.allowed.len()),
            &crate::log::Fields::default().channel("imessage"),
        );
        ...
        loop {
            match self.run_rpc(handler, &mut last_seen) {
                ...
                Err(e) => {
                    crate::log::warn_with(
                        "imessage",
                        format!("RPC failed: {}; restarting", redact(&e)),
                        &crate::log::Fields::default().channel("imessage"),
                    );
                }
            }
    ...
    fn pump(
        ...
        for line in BufReader::new(stdout).lines() {
            ...
            let working_writer = writer.clone();
            let working_target = inbound.target.clone();
            let mut working = crate::working::Working::delayed(move || {
                if let Err(e) =
                    working_writer.send(&working_target, crate::working::FALLBACK_NOTICE)
                {
                    crate::log::warn_with(
                        "imessage",
                        format!("working notice failed: {}", redact(&e)),
                        &crate::log::Fields::default().channel("imessage"),
                    );
                }
            });
            ...
            if !reply.trim().is_empty() {
                writer.send(&inbound.target, &reply)?;
            }
        }
