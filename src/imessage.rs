...
impl IMessage {
    ...
    pub fn run_rpc(&self, handler: &mut dyn FnMut(&str, &str) -> String) -> Result<(), String> {
        let mut child = self.spawn_rpc()?;
        let result = self.pump(&mut child, handler);
        let _ = child.kill();
        let _ = child.wait();
        result
    }
    ...
    fn pump(
        ...
        for line in BufReader::new(stdout).lines() {
            ...
            let reply = handler(&inbound.key, &inbound.text);
