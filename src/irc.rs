...
fn read_wire_line(stream: &Arc<Mutex<Stream>>) -> Result<String, String> {
    ...
    loop {
        match stream.read(&mut byte) {
            ...
            Ok(_) if byte[0] == b'\n' => break,
            Ok(_) if buf.len() < 8192 => buf.push(byte[0]),
            Ok(_) => {}
...
fn write_wire(stream: &Arc<Mutex<Stream>>, wire: &str) -> Result<(), String> {
    let mut stream = stream
        .lock()
        .map_err(|_| "irc: connection lock poisoned".to_string())?;
    stream
        .write_all(wire.as_bytes())
        .map_err(|e| format!("irc: write: {e}"))?;
    stream.flush().map_err(|e| format!("irc: flush: {e}"))
}

fn send_privmsg(stream: &Arc<Mutex<Stream>>, target: &str, text: &str) -> Result<(), String> {
    let wire = chunks_for(text, payload_budget(target))
        .into_iter()
...
impl Irc {
    ...
    fn session(
        ...
        let stream = Arc::new(Mutex::new(self.connect()?));
        write_wire(
            &stream,
            &format!(
                "NICK {}\r\nUSER {} 0 * :openphoenix\r\n",
        ...
        loop {
            ...
            match parse_line(&line, &self.nick) {
                Event::Ping(tok) => write_wire(&stream, &format!("PONG :{tok}\r\n"))?,
                Event::Welcome => {
                    let joins = self
                        .channels
                        .iter()
                ...
                Event::Msg {
                    ...
                    let working_stream = Arc::clone(&stream);
                    let working_target = target.clone();
                    let mut working = crate::working::Working::delayed(move || {
                        if let Err(e) = send_privmsg(
                            &working_stream,
                            &working_target,
                            crate::working::FALLBACK_NOTICE,
                        ) {
                            crate::log::warn("irc", format!("working notice failed: {e}"));
                        }
                    });
                    let reply = handler(&sender, &text);
                    working.finish();
                    send_privmsg(&stream, &target, &reply)?;
                }
                Event::Other => {}
            }
        }
        ...
        loop {
            match self.session(handler, &mut last_seen) {
                ...
                Err(e) => {
                    crate::log::warn(
                        "irc",
                        format!("connection failed: {e}; reconnecting in {backoff}s"),
                    );
