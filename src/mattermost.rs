...
impl Mattermost {
    ...
    pub fn typing_payload(channel: &str) -> Value {
        json!({"channel_id": channel})
    }

    fn typing(&self, channel: &str) -> Result<(), String> {
        ureq::post(&format!("{}/api/v4/users/me/typing", self.url))
            .set("Authorization", &format!("Bearer {}", self.token))
            .set("Content-Type", "application/json")
            .timeout(Duration::from_secs(15))
            .send_string(&Self::typing_payload(channel).to_string())
            .map_err(|e| redact(&e.to_string()))?;
        Ok(())
    }

    pub fn working(&self, channel: &str) -> crate::working::Working {
        let channel = channel.to_string();
        let mattermost = self.clone();
        crate::working::Working::native(
            crate::working::NATIVE_INTERVAL,
            move || {
                let _ = mattermost.typing(&channel);
            },
            || {},
        )
    }
                    ...
                    if !reply.is_empty() {
                        if let Err(e) = self.post(&channel, &reply) {
                            crate::log::error_with(
                                ...
                                &crate::log::Fields::default().channel("mattermost"),
                            );
                        }
                    }
                }
        ...
        loop {
            let me = match self.me() {
                ...
                Err(e) => {
                    crate::log::warn_with(
                        "mattermost",
                        format!("authentication failed: {e}; retrying in {backoff}s"),
                        &crate::log::Fields::default().channel("mattermost"),
                    );
...
mod tests {
    ...
    #[test]
    fn typing_payload_names_the_channel() {
        assert_eq!(
            Mattermost::typing_payload("c1"),
            json!({"channel_id": "c1"})
        );
    }
