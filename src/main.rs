...
fn parse_args(argv: &[String]) -> Result<Args, String> {
    ...
    while let Some(arg) = it.next() {
        ...
        match arg.as_str() {
            ...
            | "workboard" | "canvas" | "media" | "oauth" | "transcribe" | "worktrees" | "mcp"
            | "hooks" | "agents" | "commitments" | "proxy" | "attach" | "sessions" | "browser"
            | "pairing" | "sandbox" | "devices" | "qr" | "nodes" | "node" | "plugins"
            | "promos" | "dns" | "policy" | "wiki" | "fleet" | "path" | "claws"
...
[--provider KIND] [init|run PROMPT|chat|serve|doctor|jobs|tasks|sessions|skill|migrate]\n"
    ...
    s.push_str(&cmd(
        "skill",
        "search or install ClawHub skills: skill search QUERY | skill install OWNER/SLUG",
    ));
    s.push_str(&cmd(
        "service",
...
fn build_agent(cfg: &Config, interactive: bool) -> Result<Agent, String> {
    ...
    let tty = io::stdout().is_terminal();
    let on_event: tools::EventFn = Box::new(move |name: &str, args: &Value| {
        crate::spinner::notify_activity();
        let a: String = args.to_string().chars().take(120).collect();
        let (dim, accent, off) = if tty {
            ("\x1b[2m", "\x1b[38;5;208;1m", "\x1b[0m")
        } else {
            ("", "", "")
        };
        eprintln!("  {accent}\u{1f9f0}{off} {dim}{name}{off} {a}");
        if interactive {
...
pub const CHAT_COMMANDS: [&str; 34] = [
    "activation",
    "approve",
    "colab",
    "commands",
...
privacy: {cur_privacy}",
        ...
        "/colab" => {
            ...
            if arg.is_empty() {
                return CmdReply::say(
                    "usage: /colab PROVIDER/MODEL TASK\n\
example: /colab anthropic/claude-sonnet-5 write and test a fizzbuzz function\n\
your current model joins as the first speaker; the model you name here is the \
second. They alternate turns, sharing tools and workspace, until one says the \
task is done or 6 rounds pass.",
                );
            }
            let (spec, task) = match arg.split_once(char::is_whitespace) {
                Some((s, t)) if !t.trim().is_empty() => (s, t.trim()),
                _ => {
                    return CmdReply::say(
                        "usage: /colab PROVIDER/MODEL TASK (need both a model and a task)",
                    );
                }
            };
            let tty = io::stdout().is_terminal();
            ...
            return CmdReply::say(match result {
                Ok(r) => format!(
                    "{dim}colab finished: {} round(s), {}{off}\n\n{}",
                    r.rounds.len(),
                    if r.converged {
                        "converged"
                    } else {
