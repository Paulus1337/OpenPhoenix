...
fn parse_args(argv: &[String]) -> Result<Args, String> {
    ...
    while let Some(arg) = it.next() {
        ...
        match arg.as_str() {
            ...
            | "usage" | "schema" | "configure" | "status" | "dashboard" | "onboard" | "setup"
            ...
            | "exec-approvals" | "secrets" | "skills" | "logs" | "config" | "board"
            | "workboard" | "canvas" | "media" | "oauth" | "anthropic" | "claude" | "codex"
            | "transcribe" | "worktrees" | "mcp" | "hooks" | "agents" | "commitments" | "proxy"
            | "attach" | "sessions" | "browser" | "pairing" | "sandbox" | "devices" | "qr"
            | "nodes" | "node" | "plugins" | "promos" | "dns" | "policy" | "wiki" | "fleet"
            | "path" | "claws"
                if cmd.is_none() && !in_run && !in_sub =>
            {
                in_sub = true;
                sub_name = arg.clone();
            }

            "run" if cmd.is_none() && !in_run && !in_sub => {
                in_run = true;
            }
            "skill" if cmd.is_none() && !in_run && !in_sub && !in_skill => {
                in_skill = true;
            }
            "secret" if cmd.is_none() && !in_run && !in_sub && !in_skill && !in_secret => {
                in_secret = true;
            }
            "service" if cmd.is_none() && !in_run && !in_sub && !in_skill && !in_service => {
                in_service = true;
            }
            "tasks" | "flows"
                if cmd.is_none() && !in_run && !in_sub && !in_skill && !in_secret && !in_tasks =>
            {
                in_tasks = true;
            }
            other => {
    ...
    let cmd = if in_run {
        ...
        match sub_name.as_str() {
            ...
            "board" | "workboard" => Cmd::Board(sub_words),
            ...
            "oauth" | "anthropic" | "claude" => Cmd::Oauth(sub_words),
            "codex" => Cmd::Codex(sub_words),
            "transcribe" => Cmd::Transcribe(sub_words),
            "worktrees" => Cmd::Worktrees(sub_words),
            "browser" => Cmd::Browser(sub_words),
            "pairing" => Cmd::Pairing(sub_words),
            "sandbox" => Cmd::Sandbox(sub_words),
            "devices" => Cmd::Devices(sub_words),
            "qr" => Cmd::Qr(sub_words),
            "nodes" | "node" => Cmd::Nodes(sub_words),
            "plugins" => Cmd::Plugins(sub_words),
            "promos" => Cmd::Promos(sub_words),
            "dns" => Cmd::Dns(sub_words),
            "policy" => Cmd::Policy(sub_words),
            "wiki" => Cmd::Wiki(sub_words),
            "fleet" => Cmd::Fleet(sub_words),
            "path" => Cmd::PathAddr(sub_words),
            "claws" => Cmd::Claws(sub_words),
...
fn usage() -> String {
    ...
    let (accent, dim, off) = if tty {
        ("\x1b[38;5;208;1m", "\x1b[2m", "\x1b[0m")
    } else {
        ("", "", "")
    };
    let cmd = |name: &str, desc: &str| format!("  {accent}{name:<10}{off}{desc}\n");
    let mut s = format!(
        "{dim}usage:{off} phoenix [-V] [--ghost] [--recall] [--lean LEVEL] [--model NAME] \
[--provider KIND] [init|run PROMPT|chat|serve|doctor|jobs|tasks|sessions|skill|migrate]\n"
    );
    s.push_str(&cmd("init", "write sample config"));
    s.push_str(&cmd(
        "configure",
        "re-run the setup wizard on an existing nest",
    ));
    s.push_str(&cmd(
        "status",
        "one screen: config, model, channels, service, http",
    ));
    s.push_str(&cmd("dashboard", "open the web UI in a browser"));
    s.push_str(&cmd(
        "run",
        "one-shot task, ghost by default; run - reads the prompt from stdin",
    ));
    s.push_str(&cmd("chat", "interactive REPL (default)"));
    s.push_str(&cmd(
        "serve",
        "all configured channels + http api + cron jobs + dreaming",
    ));
    s.push_str(&cmd(
        "doctor",
        "audit config, permissions, and risky settings",
    ));
    s.push_str(&cmd("jobs", "list cron jobs and validate their schedules"));
    s.push_str(&cmd(
        "tasks",
        "list background tasks: tasks | tasks ID | tasks cancel ID",
    ));
    s.push_str(&cmd("sessions", "list stored serve-mode sessions"));
    s.push_str(&cmd(
        "skill",
        "search or install skills from the configured registry: skill search QUERY | skill install OWNER/SLUG",
    ));
    s.push_str(&cmd(
        "service",
        "run serve as a background service: install|uninstall|start|stop|restart|status|logs",
    ));
    s.push_str(&cmd(
        "migrate",
        "convert an AI gateway config [--from PATH] [--write] [--force] [--secrets]",
    ));
    s.push_str(&cmd(
        "update",
        "fetch the latest release, verify checksum, swap this binary [--check]",
    ));
    s.push_str(&cmd(
        "models",
        "list live models from the current provider (plus aliases)",
    ));
    s.push_str(&cmd(
        "usage",
        "show proactive account limits for the current provider",
    ));
    s.push_str(&cmd(
        "schema",
        "print the JSON Schema contract for config.toml",
    ));
    s.push_str(&cmd(
        "secret",
        "encrypted secret store: set NAME | list | rm NAME | import",
    ));
    s.push('\n');
    s.push_str(&format!(
        "  {dim}--json{off}    machine-readable output for doctor, jobs, tasks, sessions, status\n"
    ));
    s.push_str(&format!(
        "  {dim}--install-daemon{off}  with init or configure: install the background service too\n"
    ));
    s.push('\n');
    s.push_str(&format!("{accent}first time here?{off}\n"));
    s.push_str(&format!(
        "  phoenix {accent}configure{off}          answer a few questions and it sets itself up\n"
    ));
    s.push_str(&format!(
        "  phoenix {accent}doctor{off}             check config, keys, and permissions\n"
    ));
    s.push_str(&format!(
        "  phoenix {accent}run{off} \"say hello\"     one task, then it forgets\n"
    ));
    s.push_str(&format!(
        "  phoenix {accent}serve{off}              go live on your chat apps\n"
    ));
    s.push('\n');
    s.push_str("no API key yet? ollama needs none; google and nvidia have free tiers.\n");
    s.push_str(&format!("{DOCS_URL}\n"));
    s.push_str(&format!("{dim}openphoenix {VERSION}{off}"));
    s
}
...
fn cmd_secret(words: &[String]) -> u8 {
    ...
    match sub {
        ...
        "import" => {
            ...
            let (cleaned, found) = config::strip_inline_secrets(&raw);
            if found.is_empty() {
                println!("no plaintext secrets in {}", path.display());
                return 0;
            }
            let dry = words.iter().any(|w| w == "--dry-run");
            println!("plaintext secrets in {}:", path.display());
            for (name, _) in &found {
                println!("  {name}");
            }
            if dry {
                println!("\ndry run: nothing written. Re-run without --dry-run to encrypt them.");
                return 0;
            }
            let entries: Vec<(String, Vec<String>)> =
                found.iter().map(|(n, v)| (n.clone(), v.clone())).collect();
            if let Err(e) = secrets::stash_named(&entries) {
                eprintln!("error: cannot encrypt: {e}");
                return 2;
            }
            let backup = path.with_extension("toml.plaintext.bak");
            if let Err(e) = std::fs::rename(&path, &backup) {
                eprintln!("error: cannot back up {}: {e}", path.display());
                return 2;
            }
            if let Err(e) = onboard::write_config(&path, &cleaned) {
            ...
            println!("the old file is at {}", backup.display());
            println!("check phoenix doctor, then shred the backup");
            0
        }
        _ => {
            eprintln!(
                "usage: phoenix secret set NAME | secret list | secret rm NAME | secret import [--dry-run]"
            );
...
fn build_agent(cfg: &Config, interactive: bool) -> Result<Agent, String> {
    ...
    let tty = io::stdout().is_terminal();
    let on_event: tools::EventFn = std::sync::Arc::new(move |name: &str, _args: &Value| {
        crate::spinner::notify_activity();
        crate::log::debug_with(
            "tools",
            format!("tool event name={name}"),
            &crate::log::Fields::default().channel("cli"),
        );
        if interactive {
            crate::spinner::start("thinking", tty);
        }
    });
    ...
    if !cfg.mcp_servers.is_empty() {
        ...
        for problem in &problems {
            crate::log::warn("mcp", problem);
        }
        if !tools.is_empty() {
            toolbox.attach_mcp(tools);
            let names = toolbox.mcp_tool_names();
            crate::log::info("mcp", format!("{} tools available", names.len()));
        }
    }
...
fn slash(line: &str, agent: &mut Agent) -> bool {
    ...
    for _ in 0..32 {
        let Some(reply) = channel_command(Some(agent), &cfg, &line) else {
        ...
        let stdin = io::stdin();
        let picked = cli_pick(&reply, &mut stdin.lock(), &mut io::stdout());
        match picked {
            Some(cb) => line = cb,
            None => {
                println!("cancelled");
...
fn cmd_chat(mut cfg: Config) -> u8 {
    ...
    loop {
        ...
        let colab = agent.colab_on();
        if agent.stream_stdout && !colab {
            crate::spinner::start("thinking", tty);
            print!("\n{c_phx}phoenix ›{c_off} ");
            let _ = io::stdout().flush();
            ...
            let raw = if colab {
                agent.run_colab_turn(line)
            } else {
                agent.run(line)
            };
            let out = crate::text::sanitize_terminal(&raw);
            crate::spinner::stop();
            println!("\n{c_phx}phoenix ›{c_off} {out}\n");
        }
...
impl CmdReply {
    ...
    fn pairs(&self) -> Vec<(String, String)> {
        self.buttons
            ...
            .map(|(label, cb)| {
                (
                    label.trim_start_matches("\u{2705} ").trim().to_string(),
                    self.actions
                        .iter()
                        .find(|(wire, _)| wire == cb)
...
fn resolve_pick(pairs: &[(String, String)], text: &str) -> Option<String> {
    let t = text.trim();
    if t.is_empty() {
        return None;
    }
    if let Ok(n) = t.parse::<usize>() {
        return pairs.get(n.checked_sub(1)?).map(|(_, cb)| cb.clone());
    }
    let tl = t.to_ascii_lowercase();
    pairs
        .iter()
        .position(|(l, _)| plain_label(l) == tl)
        .or_else(|| {
            let hits: Vec<usize> = pairs
                .iter()
                .enumerate()
                .filter(|(_, (l, _))| plain_label(l).starts_with(&tl))
                .map(|(i, _)| i)
                .collect();
            if hits.len() == 1 {
                Some(hits[0])
...
fn render_numbered(reply: &CmdReply) -> (String, Vec<(String, String)>) {
    let pairs = reply.pairs();
    if pairs.is_empty() {
        return (reply.text.clone(), pairs);
    }
...
fn text_channel_reply(agent: Option<&mut Agent>, cfg: &Config, text: &str) -> Option<String> {
    let Some(a) = agent else {
        let reply = channel_command(None, cfg, text)?;
        return Some(if reply.buttons.is_empty() {
            reply.text
        } else {
            render_numbered(&reply).0
        });
    };
    let mut effective = text.to_string();
    if let Some(picks) = a.pending_pick.take() {
        if let Some(cb) = resolve_pick(&picks, text) {
            effective = cb;
        }
    }
    let reply = channel_command(Some(&mut *a), cfg, &effective)?;
    if let Err(error) = a.persist_preferences() {
        crate::log::error("sessions", format!("preference save failed: {error}"));
    }
    a.pending_actions = reply
        .actions
        .iter()
        .filter_map(|(wire, action)| {
            wire.strip_prefix("/pick ")
                .map(|token| (token.to_string(), action.clone()))
        })
        .collect();
    if reply.buttons.is_empty() {
        return Some(reply.text);
    }
    let (rendered, picks) = render_numbered(&reply);
    a.pending_pick = Some(picks);
    Some(rendered)
}

fn cli_pick(reply: &CmdReply, r: &mut impl BufRead, w: &mut impl Write) -> Option<String> {
    let pairs = reply.pairs();
    if pairs.is_empty() {
        return None;
    }
    let default = reply
        ...
        .iter()
        .flatten()
        .position(|(l, _)| l.starts_with("\u{2705} "))
        .unwrap_or(0);
    let items: Vec<menu::Item> = pairs
        .iter()
        .map(|(l, _)| menu::Item::new(l.clone(), ""))
        .collect();
    if let Ok(choice) = menu::select(&reply.text, &items, default) {
        return choice.map(|i| pairs[i].1.clone());
    }
    let _ = writeln!(w, "{}", reply.text);
    for (i, (l, _)) in pairs.iter().enumerate() {
        let _ = writeln!(w, "  {:>2}) {l}", i + 1);
    }
    let _ = write!(w, "pick a number or name (Enter cancels): ");
    let _ = w.flush();
    let mut ans = String::new();
    if r.read_line(&mut ans).unwrap_or(0) == 0 {
        return None;
    }
    resolve_pick(&pairs, &ans)
}

pub const CHAT_COMMANDS: [&str; 38] = [
    "activation",
    "approve",
    "colab",
    "commands",
...
fn provider_has_env_key(provider: &str) -> bool {
    ...
    config::provider_key_vars(provider)
        .iter()
        .any(|v| std::env::var(v).map(|s| !s.is_empty()).unwrap_or(false))
}

fn provider_buttons(current_provider: &str, cmd: &str) -> Vec<Vec<(String, String)>> {
    let mut order: Vec<&&str> = config::PROVIDER_KINDS.iter().collect();
    order.sort_by_key(|p| !provider_has_env_key(p));
    let mut rows: Vec<Vec<(String, String)>> = Vec::new();
    for chunk in order.chunks(3) {
        rows.push(
            chunk
                ...
                .map(|p| {
                    let p = **p;
                    let current = if p == current_provider { "✅ " } else { "" };
                    let key = if provider_has_env_key(p) && p != "ollama" {
                        "🔑 "
                    } else {
                        ""
                    };
                    (format!("{current}{key}{p}"), format!("{cmd} {p}"))
                })
                .collect(),
        );
    }
...
fn provider_model_names(cfg: &Config, provider: &str) -> Vec<String> {
    let mut c = cfg.clone();
    config::switch_provider(&mut c, provider);
    let mut names: Vec<String> = providers::list_models(&c).unwrap_or_default();
    names.sort();
    names.dedup();
    if names.is_empty() {
        names = catalog::known_models(provider)
            .iter()
            .map(|s| s.to_string())
            .collect();
    }
...
const MODEL_PAGE_SIZE: usize = 10;
...
fn parse_page_arg(arg: &str) -> (&str, usize) {
    match arg.rsplit_once(char::is_whitespace) {
        Some((prefix, raw)) if raw.starts_with("page=") => (
            prefix.trim(),
...
fn model_page(
    ...
    model_callback: impl Fn(&str) -> String,
) -> ModelPage {
    let available: Vec<&String> = names.iter().collect();
    let total_items = available.len();
    let total_pages = total_items.div_ceil(MODEL_PAGE_SIZE).max(1);
    let page = requested_page.clamp(1, total_pages);
    let start = (page - 1) * MODEL_PAGE_SIZE;
    let page_items = available
        .into_iter()
        .skip(start)
    ...
    for chunk in page_items.chunks(2) {
        rows.push(
            chunk
                .iter()
                .map(|name| {
                    let mark = if name.as_str() == marked { "✅ " } else { "" };
                    let display = catalog::display_name(name);
                    let action = model_callback(name);
                    let callback = if telegram::valid_callback(&action) {
                        action.clone()
                    } else {
                        let digest = crate::security::sha256_hex(action.as_bytes());
                        let wire = format!("/pick {}", &digest[..16]);
                        actions.push((wire.clone(), action));
                        wire
                    };
                    (
                        format!("{mark}{}", text::middle_truncate(&display, 60)),
                        callback,
                    )
                })
                .collect(),
    ...
    if total_pages > 1 {
        let mut nav = Vec::new();
        if page > 1 {
            nav.push(("◀ Prev".to_string(), page_callback(page - 1)));
        }
        nav.push((format!("{page}/{total_pages}"), page_callback(page)));
        if page < total_pages {
            nav.push(("Next ▶".to_string(), page_callback(page + 1)));
        }
        rows.push(nav);
    }
    ModelPage {
...
fn provider_model_page(
    ...
    let mut names = provider_model_names(cfg, provider);
    if provider == current_provider && !current_model.is_empty() {
        names.push(current_model.to_string());
        names.sort();
        names.dedup();
    }
    let marked = if provider == current_provider {
        current_model
    } else {
        ""
    };
    model_page(
        ...
        page,
        marked,
        |p| format!("/model {provider} page={p}"),
        |name| format!("/model_exact {provider}/{name}"),
    )
}

fn partner_model_page(
    cfg: &Config,
    provider: &str,
    current_partner: &str,
    exclude_spec: &str,
    ...
    let names: Vec<String> = provider_model_names(cfg, provider)
        .into_iter()
        .filter(|name| format!("{provider}/{name}") != exclude_spec)
        .collect();
    let marked = current_partner
    ...
    model_page(
        ...
        page,
        marked,
        |p| format!("/models partner {provider} page={p}"),
        |name| format!("/colab on {provider}/{name}"),
    )
}

fn main_provider_picker(cur_provider: &str, cur_model: &str) -> Option<CmdReply> {
    CmdReply::pick(
        format!("Current model: {cur_provider}/{cur_model}\nSelect a provider (🔑 = key found):"),
        provider_buttons(cur_provider, "/model"),
    )
}
...
fn models_menu(
    ...
    if !colab_on {
        return CmdReply::pick(
            format!(
                "{colab_status}\nMain model: {main_spec}\n\nTurn colab on and two models team up \
on every task: they split the work, use each other's strengths, and check each other's \
answers. It spends tokens with both providers each turn, but finishing faster and right \
the first time usually costs less overall."
            ),
            vec![
                vec![("🧠 Change main model".into(), "/models main".into())],
                vec![(
                    "🤝 Turn colab on: pick a partner".into(),
                    "/models partner".into(),
                )],
                vec![(
                    "⚡ Auto-pick the strongest partner".into(),
                    "/colab auto".into(),
                )],
            ],
        );
    }
    let partner = partner.unwrap_or("(unset)");
    CmdReply::pick(
        format!("{colab_status}\n\nChange the team or turn it off:"),
        vec![
            vec![(
                format!("🐦‍🔥 Primary: {}", text::middle_truncate(main_spec, 28)),
                "/models main".into(),
            )],
            vec![(
                format!("🪶 Partner: {}", text::middle_truncate(partner, 28)),
                "/models partner".into(),
            )],
            vec![("🚫 Turn colab off".into(), "/colab off".into())],
        ],
    )
}
...
fn model_arg_reply(
    ...
    let (provider_arg, requested_page) = parse_page_arg(arg);
    if !provider_arg.contains('/') && config::known_kind(provider_arg) {
        let page = provider_model_page(cfg, provider_arg, cur_provider, cur_model, requested_page);
        if page.total_items == 0 {
            return CmdReply::say(format!(
                "No models listed for {provider_arg} yet. Set one directly with /model {provider_arg}/NAME."
            ));
        }
        return CmdReply::pick_actions(
            model_picker_header(
                cfg,
                provider_arg,
                page.total_items,
    ...
    match apply_model_choice(a, arg) {
        Ok((model, note)) => CmdReply::say(format!(
            "✅ Model set\nThis chat now uses {}/{model}.{note}",
            a.cfg.provider
        )),
...
fn model_picker_header(
    ...
    let key = if provider == "ollama" {
        "local".to_string()
    } else if let Some(var) = config::provider_key_vars(provider)
        .iter()
        .find(|v| std::env::var(v).map(|s| !s.is_empty()).unwrap_or(false))
    {
        format!("🔑 {var}")
    } else if !cfg.api_key.is_empty() && provider == cfg.provider {
        "🔑 config".to_string()
    } else {
        "no key yet".to_string()
    };
    let page_line = if total_pages > 1 {
        format!(" · page {page}/{total_pages}")
    } else {
        String::new()
    };
    format!("Models ({provider} · {key}): {count} available{page_line}\npick a model (friendly name · raw id):")
}

fn split_spec_rounds(rest: &str) -> (Option<String>, u32) {
    ...
    while let Some(tok) = it.next() {
        if tok == "--rounds" {
            if let Some(n) = it.next() {
                rounds = n.parse().unwrap_or(0);
            }
        } else if spec.is_none() {
            spec = Some(tok.to_string());
        }
    }
    (spec, rounds)
}

fn apply_exact_model_choice(a: &mut Agent, provider: &str, model: &str) -> Result<String, String> {
    let before = a.cfg.provider.clone();
    if provider == before {
        a.cfg.model = model.to_string();
    } else {
        a.retarget(&format!("{provider}/{model}"))?;
    }
    Ok(model_switch_note(&a.cfg, &before))
}

fn model_switch_note(cfg: &Config, before: &str) -> String {
    if cfg.provider == before {
        return String::new();
    }
    let key_note = if cfg.api_key.is_empty() && cfg.provider != "ollama" {
...
fn apply_model_choice(a: &mut Agent, arg: &str) -> Result<(String, String), String> {
    ...
    let note = model_switch_note(&a.cfg, &before);
    Ok((a.cfg.model.clone(), note))
}

fn run_turn(a: &mut Agent, text: &str) -> String {
    crate::agent::arm_interrupt();
    let out = if a.colab_on() {
        a.run_colab_turn(text)
    } else {
        a.run(text)
    };
    crate::agent::disarm_interrupt();
...
fn channel_command(agent: Option<&mut Agent>, cfg: &Config, text: &str) -> Option<CmdReply> {
    let t = text.trim();
    let command_shape = match t.rsplit_once(char::is_whitespace) {
        Some((prefix, raw))
            if raw.strip_prefix("page=").is_some_and(|page| {
    ...
    if !looks_like_command(command_shape) {
        if is_abort_request(t) {
            crate::daemon::set_interrupt_pending(true);
            let Some(a) = agent else {
                return CmdReply::say("stopping");
            };
            let ids: Vec<u64> = a
                .toolbox
                .pending_entries()
            ...
            if n == 0 {
                return CmdReply::say("stopping the current turn");
            }
            return CmdReply::say(format!(
                "stopping the current turn: denied {n} pending command(s)"
            ));
        }
        return None;
    }
...
\n\
Options\n\
/model: change the model | /models: model and colab menu\n\
/think <level> | /reasoning on|off | /fast on|off\n\
...
/skill <name> [input]\n\
\n\
Two models as a team\n\
/models: turn colab on or off from the menu\n\
/colab on PROVIDER/MODEL | /colab auto | /colab off\n\
\n\
Approvals\n\
...
without an argument to get buttons.",
        ...
        "/reason" | "/reasoning" => {
            ...
            if config::REASONING_VISIBILITY.contains(&arg) {
                a.set_reasoning_visible(arg == "on");
                let scope = if a.colab_on() {
                    "both colab models"
                } else {
                    "this chat"
                };
                return CmdReply::say(format!(
                    "reasoning updates → {arg} ({scope})
This shows the detailed public team work log, including each seat's stated rationale and tool activity, never private chain-of-thought."
                ));
            }
            let state = if a.cfg.reasoning_visible { "on" } else { "off" };
            return CmdReply::pick(
                format!(
                    "reasoning updates: {state}
Show the detailed public team work log, never private chain-of-thought:"
                ),
                level_buttons(state, &config::REASONING_VISIBILITY, "/reasoning"),
            );
        }
        "/think" | "/thinking" => {
            let supported = agent
                .as_ref()
                .map(|a| providers::thinking_levels_for(&a.cfg))
                .unwrap_or_else(|| providers::thinking_levels_for(cfg));
            let cur_provider = agent
                .as_ref()
                .map(|a| a.cfg.provider.clone())
                .unwrap_or_else(|| cfg.provider.clone());
            let main_spec = format!("{cur_provider}/{cur_model}");
            let (target, level_arg) = match arg.split_once(char::is_whitespace) {
                Some((t, l)) if matches!(t, "main" | "partner" | "both") => (t, l.trim()),
                _ if matches!(arg, "main" | "partner" | "both") => (arg, ""),
                _ => ("", arg),
            };
            if config::THINKING_LEVELS.contains(&level_arg) {
                let Some(a) = agent else {
                    return CmdReply::say("no active chat session yet");
                };
                if target == "partner" {
                    return CmdReply::say(match a.set_partner_thinking(level_arg) {
                        Some((spec, ok)) => {
                            let note = if ok {
                                String::new()
                            } else {
                                format!("\nNote: {spec} has no adjustable thinking at that level.")
                            };
                            format!("partner thinking → {level_arg} ({spec}){note}")
                        }
                        None => {
                            "colab is off; no partner to set. Turn it on with /colab.".to_string()
                        }
                    });
                }
                a.cfg.thinking = level_arg.to_string();
                let main_tail = if supported.contains(&level_arg) {
                    ""
                } else {
                    " has no adjustable thinking; the setting rests until you switch models"
                };
                ...
                if target != "main" {
                    if let Some((spec, ok)) = a.set_partner_thinking(level_arg) {
                        let tail = if ok {
                            ""
                        } else {
                            " (no adjustable thinking at that level)"
                        };
                        lines.push(format!("partner {spec}{tail}"));
                    }
                }
                let scope = if lines.len() > 1 { "team" } else { "this chat" };
                return CmdReply::say(format!(
                    "thinking → {level_arg} ({scope})\n{}",
                    lines.join("\n")
                ));
            }
            if target == "partner" {
                ...
                let Some(levels) = a.partner_thinking_levels() else {
                    return CmdReply::say(
                        "colab is off; no partner to set. Turn it on with /colab.",
                    );
                };
                let label = a.partner_label().unwrap_or_default();
                let plevel = a.partner_thinking().unwrap_or_default();
                return CmdReply::pick(
                    format!("Partner {label} thinking: {plevel}\nLevels it supports:"),
                    level_buttons(&plevel, &levels, "/think partner"),
                );
            }
            if target == "main" {
                return CmdReply::pick(
                    format!("Main {main_spec} thinking: {cur_thinking}\nLevels it supports:"),
                    level_buttons(&cur_thinking, &supported, "/think main"),
                );
            }
            if target == "both" {
                return CmdReply::pick(
                    "Set both models to one level; each uses it only if it supports it.\nLevels:"
                        .to_string(),
                    level_buttons(&cur_thinking, &supported, "/think both"),
                );
            }
            if let Some(a) = agent.as_ref() {
                if let (Some(partner), Some(plevel)) = (a.partner_label(), a.partner_thinking()) {
                    return CmdReply::pick(
                        format!(
                            "\u{1f91d} Colab thinking. The two models support different levels, \
so set each one on its own. \"Both\" applies a level to whichever model supports it.\n\
Main {main_spec}: {cur_thinking}\nPartner {partner}: {plevel}"
                        ),
                        vec![
                            vec![(
                                format!("🐦‍🔥 Primary: {}", text::middle_truncate(&main_spec, 26)),
                                "/think main".into(),
                            )],
                            vec![(
                                format!("🪶 Partner: {}", text::middle_truncate(&partner, 26)),
                                "/think partner".into(),
                            )],
                            vec![("\u{2696} Both the same".into(), "/think both".into())],
                        ],
                    );
                }
            }
            let header = if supported.len() <= 2 {
                format!(
                    "Thinking: {cur_thinking}\n{cur_provider}/{cur_model} has no adjustable \
thinking; only these apply:"
                )
            } else {
                format!("Thinking: {cur_thinking}\nLevels {cur_provider}/{cur_model} supports:")
            };
            return CmdReply::pick(header, level_buttons(&cur_thinking, &supported, "/think"));
        }
        "/model" => {
            let cur_provider = agent
                .as_ref()
                ...
                .unwrap_or_else(|| cfg.provider.clone());
            if !arg.is_empty() {
                return model_arg_reply(agent, cfg, arg, &cur_provider, &cur_model);
            }
            return main_provider_picker(&cur_provider, &cur_model);
        }
        "/model_exact" => {
            let Some(a) = agent else {
                return CmdReply::say("That model choice expired. Open /model and choose again.");
            };
            let Some((provider, model)) = arg.split_once('/') else {
                return CmdReply::say("That model choice expired. Open /model and choose again.");
            };
            if !config::known_kind(provider) || model.is_empty() {
                return CmdReply::say("That model choice expired. Open /model and choose again.");
            }
            return CmdReply::say(match apply_exact_model_choice(a, provider, model) {
                Ok(note) => format!(
                    "✅ Model set
This chat now uses {provider}/{model}.{note}"
                ),
                Err(e) => format!("Model unchanged: {e}"),
            });
        }
        "/models" => {
            let cur_provider = agent
            ...
            let main_spec = format!("{cur_provider}/{cur_model}");
            if arg.is_empty() {
                let status = agent.as_ref().map(|a| a.colab_status()).unwrap_or_else(|| {
                    "\u{1f91d} Colab: off (one model answers alone)".to_string()
                });
                let partner = agent
                    .as_ref()
                    .and_then(|a| a.colab.as_ref())
                    .map(|c| c.partner.clone());
                return models_menu(&status, partner.is_some(), &main_spec, partner.as_deref());
            }
            if arg == "main" {
                return main_provider_picker(&cur_provider, &cur_model);
            }
            if arg == "partner" {
                return CmdReply::pick(
                    format!(
                        "Pick the partner's provider (🔑 = key found).\nThe main model stays \
{main_spec}; the partner joins it as a teammate."
                    ),
                    provider_buttons(&cur_provider, "/models partner"),
                );
            }
            if let Some(raw) = arg.strip_prefix("partner ") {
                let (provider, requested_page) = parse_page_arg(raw.trim());
                if !config::known_kind(provider) {
                    return CmdReply::say(format!(
                        "Unknown provider '{provider}'. Send /models partner to see the list."
                    ));
                }
                let cur_partner = agent
                    .as_ref()
                    .and_then(|a| a.colab.as_ref())
                    .map(|c| c.partner.clone())
                    .unwrap_or_default();
                let page =
                    partner_model_page(cfg, provider, &cur_partner, &main_spec, requested_page);
                if page.total_items == 0 {
                    return CmdReply::say(format!(
                        "No models listed for {provider} yet. Enable colab directly with /colab on {provider}/NAME."
                    ));
                }
                return CmdReply::pick_actions(
                    format!(
                        "{}\nPicking one makes it the partner and turns colab on.",
                        model_picker_header(
                            cfg,
                            provider,
                            page.total_items,
            ...
            if arg == "off" || arg == "auto" || arg == "on" || arg.starts_with("on ") {
                return channel_command(agent, cfg, &format!("/colab {arg}"));
            }
            return model_arg_reply(agent, cfg, arg, &cur_provider, &cur_model);
        }
        "/pick" => {
            ...
            let Some(action) = std::mem::take(&mut a.pending_actions).remove(arg) else {
                return CmdReply::say("That model choice expired. Open /model and choose again.");
            };
            return channel_command(Some(a), cfg, &action);
        }
        "/forget" => {
            ...
            match arg {
                "on" | "auto" => {
                    ...
                    if a.cfg.prev_model.is_empty() {
                        a.cfg.prev_model = format!("{}/{}", a.cfg.provider, a.cfg.model);
                    }
...
privacy: {cur_privacy}",
        ...
        "/colab" => {
            ...
            if arg.is_empty() {
                return CmdReply::say(format!(
                    "{}\n\n\
Easiest way: send /models and use the buttons.\n\n\
/colab on PROVIDER/MODEL [--rounds N]  turn colab mode on\n\
/colab auto                            pick the strongest available partner\n\
/colab off                             back to a single model\n\
/colab PROVIDER/MODEL TASK             one team task (colab stays off)",
                    a.colab_status()
                ));
            }
            ...
            match sub_lower.as_str() {
                "off" => {
                    let was = a.disable_colab();
                    return CmdReply::say(if was {
                        "colab is now off; back to a single model"
                    } else {
                        "colab was already off"
                    });
                }
                "on" | "auto" => {
                    let (spec, rounds) = if sub_lower == "auto" {
                        (
                            crate::colab::pick_auto_partner(&a.cfg),
                            split_spec_rounds(rest).1,
                        )
                    } else {
                        split_spec_rounds(rest)
                    };
                    let Some(spec) = spec else {
                        return CmdReply::say(if sub_lower == "auto" {
                            "colab auto found no other provider with an API key; set one up or use /colab on PROVIDER/MODEL".to_string()
                        } else {
                            "usage: /colab on PROVIDER/MODEL [--rounds N]".to_string()
                        });
                    };
                    let enabled = if sub_lower == "auto" {
                        a.enable_auto_colab(&spec, rounds)
                    } else {
                        a.enable_colab(&spec, rounds)
                    };
                    return CmdReply::say(match enabled {
                        Ok(()) => format!(
                            "🤝 Colab is ON\n🐦‍🔥 Primary: {}/{}\n🪶 Partner: {spec}\n\nSend your next \
message and both models will work on it as a team; you get one final answer with a short \
team note. /models shows progress, /colab off returns to one model.",
                            a.cfg.provider, a.cfg.model
                        ),
                        Err(e) => format!("colab not enabled: {e}"),
                    });
                }
                _ => {}
            }
            let (spec, task) = match arg.split_once(char::is_whitespace) {
                Some((s, t)) if !t.trim().is_empty() && s.contains('/') => (s, t.trim()),
                _ => {
                    return CmdReply::say(
                        "usage:\n\
/colab on PROVIDER/MODEL [--rounds N]\n\
/colab auto\n\
/colab off\n\
/colab                       (status)\n\
/colab PROVIDER/MODEL TASK   (one-shot task)",
                    );
                }
            };
            let tty = io::stdout().is_terminal();
            ...
            let result = a.run_colab(spec, task, 0, |round: &crate::colab::Round| {
                println!("\n{accent}\u{1f91d} {}{off}\n{}", round.speaker, round.text);
            });
            return CmdReply::say(match result {
                Ok(r) => {
                    let status = if r.recovery_exhausted {
                        "recovery exhausted before completion"
                    } else if r.solo {
                        "one model finished it (partner unreachable)"
                    } else if r.converged {
                        "converged"
                    } else {
                        "stopped at the round cap"
                    };
                    let note = match &r.team_note {
                        Some(n) => format!("\n{dim}note: {n}{off}"),
                        None => String::new(),
                    };
                    format!(
                        "{dim}colab finished: {} round(s), {status}{off}{note}\n\n{}",
                        r.rounds.len(),
                        r.final_text
                    )
                }
                Err(e) => format!("colab failed: {e}"),
            });
        }
        "/skill" => {
            let Some(a) = agent else {
        ...
        "/usage" => {
            return CmdReply::say(match agent {
                Some(a) => format!(
                    "{}\naccount limits: {}",
                    usage_line(&a.cfg.model, &a.usage),
                    usage::text(&a.cfg)
                ),
                None => format!(
                    "no usage yet in this chat\naccount limits: {}",
                    usage::text(cfg)
                ),
            });
        }
    ...
    if t == "/status" {
        ...
        let colab = match &agent {
            Some(a) => match &a.colab {
                Some(c) => format!(
                    "colab: on (partner {}, {} turns, {} tokens together)",
                    c.partner,
                    c.rounds_run,
                    c.tokens_primary + c.tokens_partner
                ),
                None => "colab: off".to_string(),
            },
            None => "colab: off".to_string(),
        };
        return CmdReply::say(format!(
...
fn cmd_serve(cfg: Config) -> u8 {
    crate::daemon::set_service_mode(true);
    crate::log::info("serve", format!("openphoenix {VERSION} starting"));

    let lock_path = daemon::default_path();
    let _lock = match daemon::Lock::acquire(&lock_path) {
        Ok(l) => l,
        Err(e) => {
            crate::log::error("serve", &e);
            return 2;
        }
    };
    daemon::spawn_shutdown_watchdog(std::time::Duration::from_secs(30));
    ...
    if recovered > 0 {
        crate::log::warn(
            "tasks",
            format!("recovered {recovered} task(s) left by the last run"),
        );
    }
    ...
    let tg = match Telegram::new(&cfg) {
        ...
        Err(e) => {
            ...
            crate::log::warn_with(
                "serve",
                format!("telegram disabled: {e}; serving remaining channels"),
    ...
    if let Err(e) = providers::make(&cfg) {
        crate::log::error("serve", e.to_string());
        return 2;
    }
    ...
    if cfg.http_enabled {
        if cfg.http_token.is_empty() {
            crate::log::error_with(
                "http",
                "enabled but no bearer credential is configured",
                &crate::log::Fields::default().channel("http"),
            );
            return 2;
        }
        ...
        if public && (cfg.http_web && (cfg.http_user.is_empty() || cfg.http_pass.is_empty())) {
            crate::log::error_with(
                "http",
                format!("refusing unauthenticated web UI on public bind {bind_ip}"),
                &crate::log::Fields::default().channel("http"),
            );
            return 2;
        }
        let listener = match std::net::TcpListener::bind((bind_ip.as_str(), cfg.http_port)) {
            ...
            Err(e) => {
                crate::log::error_with(
                    "http",
                    format!("bind {bind_ip}:{} failed: {e}", cfg.http_port),
                    &crate::log::Fields::default().channel("http"),
                );
                return 2;
            }
        ...
        if public {
            crate::log::warn_with(
                "http",
                format!(
                    "listening on public bind {bind_ip}:{}; use HTTPS",
                    cfg.http_port
                ),
                &crate::log::Fields::default().channel("http"),
            );
        ...
        crate::log::info_with(
            "http",
            format!("API listening on {bind_ip}:{}", cfg.http_port),
            &crate::log::Fields::default().channel("http"),
        );
    }
    ...
    if whatsapp::WhatsApp::wanted(&cfg) {
        ...
        let listener = match std::net::TcpListener::bind(("127.0.0.1", cfg.wa_webhook_port)) {
            ...
            Err(e) => {
                crate::log::error_with(
                    "whatsapp",
                    format!("webhook bind failed on port {}: {e}", cfg.wa_webhook_port),
                    &crate::log::Fields::default().channel("whatsapp"),
                );
                return 2;
            }
        };
            ...
            let mut handler = move |from: &str, text: &str| -> String {
                ...
                if !agents.contains_key(&key) {
                    match build_agent(&wa_cfg, false) {
                        Ok(mut a) => {
                            if wa_sessions {
                                sessions::restore_agent_state(&wa_sess_dir, &key, &mut a);
                            }
                            if wa_sessions {
                                a.bind_session(&wa_sess_dir, &key);
                            }
                ...
                if let Some(out) = text_channel_reply(agents.get_mut(&key), &wa_cfg, text) {
                    return out;
                }
                let Some(reply) = agents.get_mut(&key).map(|a| run_turn(a, text)) else {
                    return String::new();
                };
                if wa_sessions {
                    if let Some(a) = agents.get(&key) {
                        if let Err(e) = sessions::save_agent_state(&wa_sess_dir, &key, a) {
                            crate::log::error_with(
                                "sessions",
                                format!("save failed: {e}"),
                                &crate::log::Fields::default().session(&key),
                            );
                        }
                    }
                }
        ...
        crate::log::info_with(
            "whatsapp",
            format!("webhook listening on loopback port {}", cfg.wa_webhook_port),
            &crate::log::Fields::default().channel("whatsapp"),
        );
    }
            ...
            let mut handler = move |channel: &str, text: &str| -> String {
                ...
                if !agents.contains_key(&key) {
                    match build_agent(&dc_cfg, false) {
                        Ok(mut a) => {
                            if dc_sessions {
                                sessions::restore_agent_state(&dc_sess_dir, &key, &mut a);
                            }
                            if dc_sessions {
                                a.bind_session(&dc_sess_dir, &key);
                            }
                            agents.insert(key.clone(), a);
                        }
                        Err(e) => return format!("error: {e}"),
                ...
                if let Some(out) = text_channel_reply(agents.get_mut(&key), &dc_cfg, text) {
                    return out;
                }
                let Some(reply) = agents.get_mut(&key).map(|a| run_turn(a, text)) else {
                    return String::new();
                ...
                if dc_sessions {
                    if let Some(a) = agents.get(&key) {
                        if let Err(e) = sessions::save_agent_state(&dc_sess_dir, &key, a) {
                            crate::log::error_with(
                                "sessions",
                                format!("save failed: {e}"),
                                &crate::log::Fields::default().session(&key),
            ...
            let mut handler = move |channel: &str, thread: Option<&str>, text: &str| -> String {
                ...
                if !agents.contains_key(&key) {
                    match build_agent(&sl_cfg, false) {
                        Ok(mut a) => {
                            if sl_sessions {
                                sessions::restore_agent_state(&sl_sess_dir, &key, &mut a);
                            }
                            if sl_sessions {
                                a.bind_session(&sl_sess_dir, &key);
                            }
                            agents.insert(key.clone(), a);
                        }
                        Err(e) => return format!("error: {e}"),
                ...
                if let Some(out) = text_channel_reply(agents.get_mut(&key), &sl_cfg, text) {
                    return out;
                }
                let Some(reply) = agents.get_mut(&key).map(|a| run_turn(a, text)) else {
                    return String::new();
                ...
                if sl_sessions {
                    if let Some(a) = agents.get(&key) {
                        if let Err(e) = sessions::save_agent_state(&sl_sess_dir, &key, a) {
                            crate::log::error_with(
                                "sessions",
                                format!("save failed: {e}"),
                                &crate::log::Fields::default().session(&key),
                    ...
                    let mut handler = move |sender: &str, text: &str| -> String {
                        ...
                        if !agents.contains_key(&key) {
                            match build_agent(&mx_cfg, false) {
                                Ok(mut a) => {
                                    if mx_sessions {
                                        sessions::restore_agent_state(&mx_sess_dir, &key, &mut a);
                                    }
                                    if mx_sessions {
                                        a.bind_session(&mx_sess_dir, &key);
                                    }
                                    agents.insert(key.clone(), a);
                                }
                                Err(e) => return format!("error: {e}"),
                        ...
                        if let Some(out) = text_channel_reply(agents.get_mut(&key), &mx_cfg, text) {
                            return out;
                        }
                        let Some(reply) = agents.get_mut(&key).map(|a| run_turn(a, text)) else {
                            return String::new();
                        ...
                        if mx_sessions {
                            if let Some(a) = agents.get(&key) {
                                if let Err(e) = sessions::save_agent_state(&mx_sess_dir, &key, a) {
                                    crate::log::error_with(
                                        "sessions",
                                        format!("save failed: {e}"),
                                        &crate::log::Fields::default().session(&key),
                    ...
                    let mut handler = move |sender: &str, text: &str| -> String {
                        ...
                        if !agents.contains_key(&key) {
                            match build_agent(&mm_cfg, false) {
                                Ok(mut a) => {
                                    if mm_sessions {
                                        sessions::restore_agent_state(&mm_sess_dir, &key, &mut a);
                                    }
                                    if mm_sessions {
                                        a.bind_session(&mm_sess_dir, &key);
                                    }
                                    agents.insert(key.clone(), a);
                                }
                                Err(e) => return format!("error: {e}"),
                        ...
                        if let Some(out) = text_channel_reply(agents.get_mut(&key), &mm_cfg, text) {
                            return out;
                        }
                        let Some(reply) = agents.get_mut(&key).map(|a| run_turn(a, text)) else {
                            return String::new();
                        ...
                        if mm_sessions {
                            if let Some(a) = agents.get(&key) {
                                if let Err(e) = sessions::save_agent_state(&mm_sess_dir, &key, a) {
                                    crate::log::error_with(
                                        "sessions",
                                        format!("save failed: {e}"),
                                        &crate::log::Fields::default().session(&key),
                    ...
                    let mut handler = move |sender: &str, text: &str| -> String {
                        ...
                        if !agents.contains_key(&key) {
                            match build_agent(&ic_cfg, false) {
                                Ok(mut a) => {
                                    if ic_sessions {
                                        sessions::restore_agent_state(&ic_sess_dir, &key, &mut a);
                                    }
                                    if ic_sessions {
                                        a.bind_session(&ic_sess_dir, &key);
                                    }
                                    agents.insert(key.clone(), a);
                                }
                                Err(e) => return format!("error: {e}"),
                        ...
                        if let Some(out) = text_channel_reply(agents.get_mut(&key), &ic_cfg, text) {
                            return out;
                        }
                        let Some(reply) = agents.get_mut(&key).map(|a| run_turn(a, text)) else {
                            return String::new();
                        ...
                        if ic_sessions {
                            if let Some(a) = agents.get(&key) {
                                if let Err(e) = sessions::save_agent_state(&ic_sess_dir, &key, a) {
                                    crate::log::error_with(
                                        "sessions",
                                        format!("save failed: {e}"),
                                        &crate::log::Fields::default().session(&key),
            ...
            let mut handler = move |sender: &str, text: &str| -> String {
                ...
                if !agents.contains_key(&key) {
                    match build_agent(&sg_cfg, false) {
                        Ok(mut a) => {
                            if sg_sessions {
                                sessions::restore_agent_state(&sg_sess_dir, &key, &mut a);
                            }
                            if sg_sessions {
                                a.bind_session(&sg_sess_dir, &key);
                            }
                            agents.insert(key.clone(), a);
                        }
                        Err(e) => return format!("error: {e}"),
                ...
                if let Some(out) = text_channel_reply(agents.get_mut(&key), &sg_cfg, text) {
                    return out;
                }
                let Some(reply) = agents.get_mut(&key).map(|a| run_turn(a, text)) else {
                    return String::new();
                ...
                if sg_sessions {
                    if let Some(a) = agents.get(&key) {
                        if let Err(e) = sessions::save_agent_state(&sg_sess_dir, &key, a) {
                            crate::log::error_with(
                                "sessions",
                                format!("save failed: {e}"),
                                &crate::log::Fields::default().session(&key),
            ...
            let mut handler = move |chat: &str, text: &str| -> String {
                ...
                if !agents.contains_key(&key) {
                    match build_agent(&im_cfg, false) {
                        Ok(mut a) => {
                            if im_sessions {
                                sessions::restore_agent_state(&im_sess_dir, &key, &mut a);
                            }
                            if im_sessions {
                                a.bind_session(&im_sess_dir, &key);
                            }
                            agents.insert(key.clone(), a);
                        }
                        Err(e) => return format!("error: {e}"),
                ...
                if let Some(out) = text_channel_reply(agents.get_mut(&key), &im_cfg, text) {
                    return out;
                }
                let Some(reply) = agents.get_mut(&key).map(|a| run_turn(a, text)) else {
                    return String::new();
                ...
                if im_sessions {
                    if let Some(a) = agents.get(&key) {
                        if let Err(e) = sessions::save_agent_state(&im_sess_dir, &key, a) {
                            crate::log::error_with(
                                "sessions",
                                format!("save failed: {e}"),
                                &crate::log::Fields::default().session(&key),
        ...
        move |job, result| {
            if !job.webhook.is_empty() {
                if let Err(e) = scheduler::post_webhook(&job.webhook, &job.name, result) {
                    crate::log::error(
                        "scheduler",
                        format!("job webhook failed name={}: {e}", job.name),
                    );
                }
                return;
            }
            let Some(tg) = &tg_deliver else {
                crate::log::info(
                    "scheduler",
                    format!("job completed with no delivery channel name={}", job.name),
                );
                return;
            };
    ...
    let _heartbeat = heartbeat::Heartbeat::start(
        ...
        move |result| {
            let Some(tg) = &tg_hb else {
                crate::log::info("heartbeat", "completed with no delivery channel");
                return;
            };
    ...
    if cfg.heartbeat_minutes > 0 {
        crate::log::info(
            "heartbeat",
            format!("scheduled every {} min", cfg.heartbeat_minutes),
        );
    }
    ...
    if cfg.update_check_hours > 0 {
        ...
        std::thread::spawn(move || loop {
            ...
            match update::run(true) {
                Ok(_) => {}
                Err(e) => crate::log::warn("update", format!("check failed: {e}")),
            }
        });
        crate::log::info(
            "update",
            format!(
                "checks scheduled every {}h; apply disabled",
        ...
        std::thread::spawn(move || {
            ...
            loop {
                ...
                if let Err(e) = appended {
                    crate::log::error("dreaming", format!("journal write failed: {e}"));
                } else {
                    crate::log::info("dreaming", "idle reflection completed");
                }
            }
        });
        crate::log::info(
            "dreaming",
            format!("scheduled after {} idle min", cfg.dream_minutes),
        );
    }
    ...
    let Some(tg) = tg else {
        ...
        crate::log::info("serve", "stop requested; shutting down cleanly");
        return 0;
    };
    let tg_out = tg.clone();
    let progress = std::sync::Arc::new(std::sync::Mutex::new(None::<(String, i64)>));
    let progress_at = std::sync::Arc::new(std::sync::Mutex::new(std::time::Instant::now()));
    let tg_prog = tg.clone();
    let tg_watch = tg.clone();
    let result = tg.serve(
        &mut |chat_id, thread_id, text, media| {
            ...
            if !agents.contains_key(skey.as_str()) {
                match build_agent(&serve_cfg, false) {
                    Ok(mut a) => {
                        if use_sessions {
                            sessions::restore_agent_state(&sess_dir, &skey, &mut a);
                        }
                        if use_sessions {
                            a.bind_session(&sess_dir, &skey);
                        }
                        ...
                        let board = std::sync::Arc::new(std::sync::Mutex::new(
                            crate::progress::ProgressBoard::new(a.cfg.reasoning_visible),
                        ));
                        let trace = serve_cfg.trace == "on" || serve_cfg.trace == "raw";
                        a.toolbox
                            .set_event_hook(std::sync::Arc::new(move |name, args| {
                                if trace {
                                    crate::log::debug_with(
                                        "tools",
                                        format!("tool event name={name}"),
                                ...
                                match slot.as_ref() {
                                    None => {
                                        if let Some(id) = tgp.progress_start(&chat, pthread, &line)
                                        {
                                            ...
                                            *at.lock().unwrap_or_else(|error| error.into_inner()) =
            ...
            let mut effective = text.to_string();
            if let Some(a) = agents.get_mut(skey.as_str()) {
                if let Some(picks) = a.pending_pick.take() {
                    if let Some(cb) = resolve_pick(&picks, text) {
                        effective = cb;
                    }
            ...
            if let Some(reply) =
                channel_command(agents.get_mut(skey.as_str()), &serve_cfg, &effective)
            {
                if let Some(agent) = agents.get(skey.as_str()) {
                    if let Err(error) = agent.persist_preferences() {
                        crate::log::error_with(
                            "sessions",
                            format!("preference save failed: {error}"),
                            &crate::log::Fields::default().session(&skey),
                        );
                    }
                }
                if reply.buttons.is_empty() {
                ...
                if let Some(a) = agents.get_mut(skey.as_str()) {
                    a.pending_pick = Some(reply.pairs());
                    a.pending_actions = reply
                        .actions
                        .iter()
                        .filter_map(|(wire, action)| {
            ...
            let before_max = agent
                ...
                .map(|(id, _)| *id)
                .unwrap_or(0);
            let watch_stop = std::sync::Arc::new(std::sync::atomic::AtomicBool::new(true));
            let watcher = {
                let stop = watch_stop.clone();
                let tgw = tg_watch.clone();
                let typing_chat = chat_id.to_string();
                let typing_thread = thread_id;
                std::thread::spawn(move || {
                    while stop.load(std::sync::atomic::Ordering::SeqCst) {
                        if tgw.abort_requested(&typing_chat, typing_thread) {
                            crate::daemon::set_interrupt_pending(true);
                            return;
                        }
                        tgw.typing(&typing_chat, typing_thread);
                        for _ in 0..20 {
            ...
            crate::agent::arm_interrupt();
            let reply = if agent.colab_on() {
                agent.run_colab_turn_with_media(text, media)
            } else {
                agent.run_with_media(text, media)
            };
            crate::agent::disarm_interrupt();
            watch_stop.store(false, std::sync::atomic::Ordering::SeqCst);
            let _ = watcher.join();
            if let Some((c, id)) = progress
                .lock()
                .unwrap_or_else(|error| error.into_inner())
                .take()
            {
            ...
            if use_sessions {
                if let Some(a) = agents.get(skey.as_str()) {
                    if let Err(e) = sessions::save_agent_state(&sess_dir, &skey, a) {
                        crate::log::error_with(
                            "sessions",
                            format!("save failed: {e}"),
                            &crate::log::Fields::default().session(&skey),
...
fn cmd_board(words: &[String], json: bool) -> u8 {
    let path = config::home().join("board.json");
    let json = json || words.iter().any(|w| w == "--json");
    let words: Vec<String> = words.iter().filter(|w| *w != "--json").cloned().collect();
    ...
    match sub {
        "show" => {
            let Some(id) = words
                ...
                eprintln!("usage: phoenix board show ID");
                return 2;
            };
            match board::show(&path, id) {
                Ok(t) => {
                    print!("{t}");
                    0
                }
        ...
        "dispatch" => {
            let limit = words
                ...
                .unwrap_or(5)
                .clamp(1, 50);
            print!("{}", board::dispatch_text(&board::ready(&path, limit)));
            0
        }
        "list" if json => match board::list_json(&path, words.get(1).map(String::as_str)) {
            Ok(t) => {
                print!("{t}");
                0
            }
        ...
        other => {
            eprintln!(
                "unknown board subcommand '{other}': list [STATUS] | show ID | add TITLE | update ID STATUS | dispatch [N]"
            );
            2
        }
    }
...
fn cmd_oauth(words: &[String]) -> u8 {
    match words.first().map(String::as_str).unwrap_or("show") {
        "login" | "signin" => {
            let lg = oauth::begin_login();
            println!(
                "Sign in to Anthropic (Claude) with your Claude subscription; no API key needed."
            );
            println!(
            ...
            println!(
                "\n  2) After approving, Anthropic shows a code (it looks like CODE#STATE).\n     \
Copy it and paste it here."
            ...
            if io::stdin().read_line(&mut line).is_err() {
                eprintln!("could not read the pasted code");
                return 2;
            }
            let pasted = line.trim();
            if pasted.is_empty() {
                eprintln!("no code entered; nothing changed");
                return 2;
            }
            let (code, state) = match pasted.split_once('#') {
                Some((c, s)) => (c.trim().to_string(), s.trim().to_string()),
                None => (pasted.to_string(), lg.state.clone()),
            };
            match oauth::exchange(&code, &state, &lg.verifier) {
                Ok(t) => match oauth::save(&t) {
                    Ok(()) => {
                        println!(
                            "\n\u{2705} Signed in to Anthropic; tokens stored at {}.",
                            oauth::store_path().display()
                        );
                        println!(
                            "Use it: phoenix --provider anthropic --model claude-fable-5, or /model anthropic/claude-fable-5 in chat."
                        );
                        println!(
                            "Limits: {}",
                            usage::text(&Config {
                    ...
                    Err(e) => {
                        eprintln!("signed in but could not save tokens: {e}");
                        2
                    }
                },
                Err(e) => {
                    eprintln!("anthropic login failed: {e}");
                    2
                }
            }
        }
        "import" => match oauth::import_from_claude_cli() {
            Some(t) => match oauth::save(&t) {
                Ok(()) => {
                    let refreshable = if t.refresh.is_empty() {
                        " (access token only; sign in again when it expires)"
                    } else {
                        " (with refresh token; auto-renews)"
                    };
                    println!(
                        "Imported Anthropic OAuth into {}{refreshable}.",
                        oauth::store_path().display()
                    );
                    0
                }
                Err(e) => {
                    eprintln!("could not save imported tokens: {e}");
                    2
                }
            },
            None => {
                eprintln!(
                    "no Claude CLI credentials found (~/.claude/.credentials.json or \
CLAUDE_CODE_OAUTH_TOKEN); run: phoenix anthropic login"
                );
                2
            }
        },
        "logout" => {
            let _ = std::fs::remove_file(oauth::store_path());
            println!(
                "signed out of Anthropic; removed {}",
                oauth::store_path().display()
            );
            0
        }
        ...
        "usage" | "limits" => cmd_provider_usage(
            &Config {
                provider: "anthropic".into(),
                ..Config::default()
            },
        ...
        "refresh" => match oauth::refresh_access() {
            Ok(Some(_)) => {
                println!("access token refreshed and saved");
                0
            }
            Ok(None) => {
                eprintln!("could not refresh; token endpoint returned no access token");
                2
            }
            Err(error) => {
                eprintln!("could not refresh: {}", crate::security::redact(&error));
                2
            }
        },
        other => {
            eprintln!(
                "unknown anthropic subcommand '{other}': login | import | status | usage | refresh | logout"
            );
            2
        }
    }
...
fn cmd_codex(words: &[String]) -> u8 {
    match words.first().map(String::as_str).unwrap_or("status") {
        "login" | "signin" => {
            println!("Signing in to OpenAI with your ChatGPT plan (no API key needed).");
            let result = codex::login(|dc| {
                println!("\n  Open this link and sign in with ChatGPT:");
                println!("    {}", dc.verification_url);
                println!("  Enter this code when asked:");
                println!("    {}\n", dc.user_code);
                println!("  Waiting for you to approve it (up to 15 minutes)...");
            });
            match result {
                Ok(t) => {
                    let who = if t.account_id.is_empty() {
                        String::new()
                    } else {
                        format!(" (account {})", t.account_id)
                    };
                    println!("\n\u{2705} Signed in to OpenAI Codex{who}.");
                    println!(
                        "Use it: phoenix --model openai/gpt-5.6-sol, or /model openai/gpt-5.6-sol in chat."
                    );
                    println!(
                        "Limits: {}",
                        usage::text(&Config {
        ...
        "status" | "show" => match codex::load() {
            Some(t) => {
                let left = t.expires_at_ms.saturating_sub(codex::now_ms()) / 1000;
                println!("stored at {}", codex::store_path().display());
                println!(
                    "  access token   {}",
                    if t.access.is_empty() {
                        "missing"
            ...
            None => {
                println!("no ChatGPT sign-in stored yet. Run: phoenix codex login");
                0
            }
        },
        "usage" | "limits" => cmd_provider_usage(
        ...
        "refresh" => match codex::load() {
            Some(t) if !t.refresh.is_empty() => match codex::refresh(&t) {
                Ok(new) => {
                    let _ = codex::save(&new);
                    println!(
                        "refreshed; expires in {}s",
                        (new.expires_at_ms.saturating_sub(codex::now_ms())) / 1000
                    );
                    0
                }
                Err(e) => {
            ...
            _ => {
                eprintln!("nothing to refresh; run: phoenix codex login");
                2
            }
        },
        "logout" => {
            let _ = std::fs::remove_file(codex::store_path());
            println!("signed out of OpenAI Codex; the stored token was removed.");
            0
        }
        other => {
            eprintln!(
                "unknown codex subcommand '{other}': login | status | usage | refresh | logout"
            );
            2
        }
    }
...
fn browser_usage() -> String {
    "usage: phoenix browser SUBCOMMAND\n  open URL           start the browser and load a page\n  snapshot           print the accessibility outline with refs\n  click REF          click the element behind a ref\n  type REF TEXT      type into the element behind a ref (--submit presses Enter)\n  screenshot [PATH]  save a png of the current page\n  status             report whether the browser is reachable\n"
        .to_string()
}

fn browser_session(cfg: &Config, url: &str) -> Result<crate::browser::Browser, String> {
    let mut b = crate::browser::Browser::start(cfg)?;
    if !url.is_empty() {
        let out = b.navigate(url)?;
        println!("{out}");
    }
    Ok(b)
}

fn cmd_browser(cfg: &Config, words: &[String]) -> u8 {
    if !cfg.browser_enabled {
        eprintln!("browser is disabled; set [browser] enabled = true in the config first");
        return 2;
    }
    let sub = words.first().map(String::as_str).unwrap_or("status");
    let rest: Vec<&str> = words.iter().skip(1).map(String::as_str).collect();
    let submit = rest.contains(&"--submit");
    let rest: Vec<&str> = rest.into_iter().filter(|w| *w != "--submit").collect();
    match sub {
        "status" => {
            if !cfg.browser_cdp_url.is_empty() {
                match crate::browser::check_cdp_host(&cfg.browser_cdp_url) {
                    Ok(()) => println!("browser: attaching to {}", cfg.browser_cdp_url),
                    Err(e) => {
                        eprintln!("browser: cdp url refused: {e}");
                        return 2;
                    }
                }
            } else {
                ...
                let present = std::path::Path::new(&binary).exists();
                println!(
                    "browser: {binary} ({}), headless={}",
        ...
        "open" | "navigate" => {
            let Some(url) = rest.first() else {
                eprintln!("usage: phoenix browser open URL");
                return 2;
            };
            match browser_session(cfg, url) {
                Ok(mut b) => match b.snapshot() {
                    Ok(text) => {
                        println!("{text}");
                        0
                    }
        ...
        "snapshot" => {
            let url = rest.first().copied().unwrap_or("");
            match browser_session(cfg, url).and_then(|mut b| b.snapshot()) {
                Ok(text) => {
                    println!("{text}");
                    0
                }
        ...
        "click" => {
            let (Some(url), Some(r)) = (rest.first(), rest.get(1)) else {
                eprintln!("usage: phoenix browser click URL REF");
                return 2;
            };
            let done = browser_session(cfg, url).and_then(|mut b| {
                b.snapshot()?;
                b.click(r)
            });
            match done {
        ...
        "type" => {
            let (Some(url), Some(r)) = (rest.first(), rest.get(1)) else {
                eprintln!("usage: phoenix browser type URL REF TEXT [--submit]");
                return 2;
            };
            let text = rest[2..].join(" ");
            let done = browser_session(cfg, url).and_then(|mut b| {
                b.snapshot()?;
                b.type_text(r, &text, submit)
            });
            match done {
                Ok(out) => {
                    println!("{out}");
        ...
        "screenshot" => {
            let Some(url) = rest.first() else {
                eprintln!("usage: phoenix browser screenshot URL [PATH]");
                return 2;
            };
            let out = rest
                .get(1)
                .map(std::path::PathBuf::from)
                .unwrap_or_else(|| cfg.workspace.join("browser-shot.png"));
            match browser_session(cfg, url).and_then(|mut b| b.screenshot_png()) {
                Ok(png) => match crate::security::write_atomic(&out, &png, Some(0o600)) {
                    Ok(()) => {
                        println!("{} bytes written to {}", png.len(), out.display());
                        0
                    }
                    Err(e) => {
                        eprintln!("{e}");
...
fn cmd_pairing(cfg: &Config, words: &[String]) -> u8 {
    let path = pairing::store_path();
    match words.first().map(String::as_str).unwrap_or("list") {
        "list" => {
            if !cfg.pairing_enabled {
                println!("pairing is off; set [pairing] enabled = true to accept requests");
            }
            print!(
                "{}",
                pairing::list_text(&pairing::load(&path), pairing::now_ms())
            );
            0
        }
        "approve" => {
            let Some(key) = words.get(1) else {
                eprintln!("usage: phoenix pairing approve ID|CODE");
                return 2;
            };
            match pairing::take(&path, key) {
                Ok(req) => {
                    println!("approved #{} on {}: {}", req.id, req.channel, req.sender);
                    println!("{}", pairing::approve_hint(&req));
                    0
                }
                Err(e) => {
                    eprintln!("{e}");
        ...
        "deny" | "reject" => {
            let Some(key) = words.get(1) else {
                eprintln!("usage: phoenix pairing deny ID|CODE");
                return 2;
            };
            match pairing::take(&path, key) {
                Ok(req) => {
                    println!("denied #{} on {}: {}", req.id, req.channel, req.sender);
                    0
                }
                Err(e) => {
                    eprintln!("{e}");
        ...
        other => {
            eprintln!(
                "unknown pairing subcommand '{other}': list | approve ID|CODE | deny ID|CODE"
            );
            2
        }
    }
...
fn cmd_sandbox(cfg: &Config, words: &[String]) -> u8 {
    let p = sandbox::policy(cfg);
    match words.first().map(String::as_str).unwrap_or("status") {
        "status" | "show" => {
            print!("{}", sandbox::status_text(&p));
            u8::from(p.enabled() && !sandbox::runtime_available(&p.runtime))
        }
        "check" => {
            let problems = sandbox::validate(&p);
            if problems.is_empty() {
                println!("sandbox policy is consistent");
                return 0;
            }
            for w in &problems {
                eprintln!("{w}");
        ...
        "args" => {
            if !p.enabled() {
                eprintln!("sandbox is off; nothing would be run in a container");
                return 2;
            }
            let command = words[1..].join(" ");
            if command.trim().is_empty() {
                eprintln!("usage: phoenix sandbox args COMMAND");
                return 2;
            }
            match p.args(&cfg.workspace, &command) {
                Ok(args) => {
                    println!("{} {}", p.runtime, args.join(" "));
                    0
                }
                Err(e) => {
                    eprintln!("{e}");
        ...
        other => {
            eprintln!("unknown sandbox subcommand '{other}': status | check | args COMMAND");
            2
        }
    }
}

fn cmd_devices(words: &[String]) -> u8 {
    let path = devices::store_path();
    match words.first().map(String::as_str).unwrap_or("list") {
        "list" => {
            print!(
                "{}",
                devices::list_text(&devices::load(&path), devices::now_ms())
            );
            0
        }
        "add" | "pair" => {
            let Some(name) = words.get(1) else {
                eprintln!("usage: phoenix devices add NAME [read|act]");
                return 2;
            };
            let scope = words.get(2).map(String::as_str).unwrap_or("read");
            match devices::add(&path, name, scope) {
                Ok((dev, token)) => {
                    println!("paired #{} {} ({})", dev.id, dev.name, dev.scope);
        ...
        "rotate" => {
            let Some(key) = words.get(1) else {
                eprintln!("usage: phoenix devices rotate NAME|ID");
                return 2;
            };
            match devices::rotate(&path, key) {
                Ok((dev, token)) => {
                    println!("rotated #{} {}", dev.id, dev.name);
                    println!("token (shown once): {token}");
                    0
                }
                Err(e) => {
        ...
        "revoke" => {
            let Some(key) = words.get(1) else {
                eprintln!("usage: phoenix devices revoke NAME|ID");
                return 2;
            };
            match devices::revoke(&path, key) {
                Ok(dev) => {
                    println!("revoked #{} {}", dev.id, dev.name);
                    0
                }
                Err(e) => {
                    eprintln!("{e}");
        ...
        "remove" | "forget" => {
            let Some(key) = words.get(1) else {
                eprintln!("usage: phoenix devices remove NAME|ID");
                return 2;
            };
            match devices::remove(&path, key) {
                Ok(dev) => {
                    println!("removed #{} {}", dev.id, dev.name);
                    0
                }
                Err(e) => {
                    eprintln!("{e}");
        ...
        other => {
            eprintln!(
                "unknown devices subcommand '{other}': list | add NAME [SCOPE] | rotate KEY | revoke KEY | remove KEY"
            );
            2
        }
    }
...
fn cmd_qr(cfg: &Config, words: &[String]) -> u8 {
    let plain = words.iter().any(|w| w == "--large");
    let rest: Vec<&str> = words
        .iter()
        .map(String::as_str)
        .filter(|w| *w != "--large")
        .collect();
    let payload = match rest.first().copied() {
        Some("pair") | None => {
            let name = rest.get(1).copied().unwrap_or("mobile");
            match devices::add(&devices::store_path(), name, "act") {
                Ok((dev, token)) => {
                    println!("paired #{} {} ({})", dev.id, dev.name, dev.scope);
                    format!("{}?device={}&token={token}", dashboard_url(cfg), dev.name)
                }
                Err(e) => {
                    eprintln!("{e}");
                    return 2;
        ...
        Some("url") => dashboard_url(cfg),
        Some("text") => {
            let text = rest[1..].join(" ");
            if text.trim().is_empty() {
                eprintln!("usage: phoenix qr text TEXT");
                return 2;
            }
            text
        }
        Some(other) => {
            eprintln!("unknown qr subcommand '{other}': pair [NAME] | url | text TEXT");
            return 2;
        }
    };
...
fn cmd_acp(cfg: Config) -> u8 {
    let mut agent = match build_agent(&cfg, false) {
        Ok(a) => a,
        Err(e) => {
            eprintln!("error: {e}");
            return 2;
    ...
    match acp::serve(reader, &mut out, &mut |prompt| agent.run(prompt)) {
        Ok(()) => 0,
        Err(e) => {
            eprintln!("acp: {e}");
            2
...
fn cmd_nodes(words: &[String]) -> u8 {
    let path = nodes::store_path();
    let admin = words.iter().any(|w| w == "--admin");
    let rest: Vec<&str> = words
        .iter()
        .map(String::as_str)
        .filter(|w| *w != "--admin")
    ...
    match rest.first().copied().unwrap_or("list") {
        "list" | "status" | "pending" => {
            print!("{}", nodes::list_text(&nodes::load(&path)));
            0
        }
        "enroll" | "add" => {
            let Some(name) = rest.get(1) else {
                eprintln!("usage: phoenix nodes enroll NAME [ADDRESS] [CAP,CAP]");
                return 2;
            };
            let address = rest.get(2).copied().unwrap_or("");
            let caps = match nodes::parse_capabilities(rest.get(3).copied().unwrap_or("")) {
                Ok(c) => c,
                Err(e) => {
                    eprintln!("{e}");
                    return 2;
            ...
            match nodes::enroll(&path, name, address, &caps) {
                Ok(n) => {
                    println!("enrolled #{} {} (pending)", n.id, n.name);
                    if nodes::needs_admin(&n.capabilities) {
                        println!("it asks for an admin capability; approve with --admin");
                    }
                    0
                }
                Err(e) => {
        ...
        "approve" => {
            let Some(key) = rest.get(1) else {
                eprintln!("usage: phoenix nodes approve ID|NAME [--admin]");
                return 2;
            };
            match nodes::approve(&path, key, admin) {
                Ok(n) => {
                    println!("approved #{} {}", n.id, n.name);
                    0
                }
                Err(e) => {
                    eprintln!("{e}");
        ...
        "reject" | "remove" => {
            let Some(key) = rest.get(1) else {
                eprintln!("usage: phoenix nodes reject ID|NAME");
                return 2;
            };
            match nodes::reject(&path, key) {
                Ok(n) => {
                    println!("removed #{} {}", n.id, n.name);
                    0
                }
                Err(e) => {
                    eprintln!("{e}");
        ...
        "rename" => {
            let (Some(key), Some(name)) = (rest.get(1), rest.get(2)) else {
                eprintln!("usage: phoenix nodes rename ID|NAME NEWNAME");
                return 2;
            };
            match nodes::rename(&path, key, name) {
                Ok(n) => {
                    println!("renamed #{} to {}", n.id, n.name);
                    0
                }
                Err(e) => {
                    eprintln!("{e}");
        ...
        "can" => {
            let (Some(key), Some(cap)) = (rest.get(1), rest.get(2)) else {
                eprintln!("usage: phoenix nodes can NAME CAPABILITY");
                return 2;
            };
            if nodes::allows(&nodes::load(&path), key, cap) {
                println!("yes: {key} may use {cap}");
                0
            } else {
                println!("no: {key} may not use {cap}");
                1
            }
        }
        "describe" | "show" => {
            let Some(key) = rest.get(1) else {
                eprintln!("usage: phoenix nodes describe ID|NAME");
                return 2;
            };
            match nodes::describe(&nodes::load(&path), key) {
                Some(text) => {
                    print!("{text}");
                    0
                }
                None => {
                    eprintln!("no node matches '{key}'");
                    2
                }
            }
        }
        other => {
            eprintln!(
                "unknown nodes subcommand '{other}': list | enroll NAME [ADDRESS] [CAPS] | approve KEY [--admin] | reject KEY | rename KEY NAME | describe KEY | can KEY CAP"
            );
            2
        }
    }
...
fn cmd_plugins(cfg: &Config, words: &[String], json: bool) -> u8 {
    match words.first().map(String::as_str).unwrap_or("list") {
        "list" => {
            let inv = plugins::inventory(cfg);
            print!("{}", plugins::list_text(&inv, json));
            u8::from(!plugins::problems(&inv).is_empty())
        }
        "doctor" | "check" => {
            let problems = plugins::problems(&plugins::inventory(cfg));
            if problems.is_empty() {
                println!("every extension looks loadable");
                return 0;
            }
            for p in &problems {
                eprintln!("{p}");
        ...
        other => {
            eprintln!("unknown plugins subcommand '{other}': list | doctor");
            2
        }
    }
}

fn cmd_promos(cfg: &Config, words: &[String]) -> u8 {
    let promos = match promos::fetch(cfg) {
        Ok(p) => p,
        Err(e) => {
            eprintln!("{e}");
            return 2;
    ...
    match words.first().map(String::as_str).unwrap_or("list") {
        "list" => {
            print!("{}", promos::list_text(&promos));
            0
        }
        "claim" => {
            let Some(id) = words.get(1) else {
                eprintln!("usage: phoenix promos claim ID");
                return 2;
            };
            let Some(promo) = promos::find(&promos, id) else {
                eprintln!("no promotion named '{id}'; run `phoenix promos` to list them");
                return 2;
            };
            println!("{} on {} {}", promo.id, promo.provider, promo.model);
            if !promo.summary.is_empty() {
                println!("{}", promo.summary);
            }
            println!("\nto claim it:");
            for (i, step) in promos::claim_plan(promo).iter().enumerate() {
                println!("  {}. {step}", i + 1);
            }
            println!("\nphoenix never writes a key into the config for you.");
            0
        }
        other => {
            eprintln!("unknown promos subcommand '{other}': list | claim ID");
            2
        }
    }
}

fn cmd_dns(cfg: &Config, words: &[String]) -> u8 {
    match words.first().map(String::as_str).unwrap_or("plan") {
        "plan" | "setup" => {
            let domain = words
                .get(1)
                .map(String::as_str)
                .unwrap_or("phoenix.internal");
            match dns::plan_text(cfg, domain) {
                Ok(text) => {
                    print!("{text}");
                    0
                }
        ...
        "check" | "resolve" => {
            let Some(host) = words.get(1) else {
                eprintln!("usage: phoenix dns check HOST");
                return 2;
            };
            match dns::check_text(host, cfg.allow_private_network) {
                Ok(text) => {
                    print!("{text}");
                    0
                }
        ...
        other => {
            eprintln!("unknown dns subcommand '{other}': plan [DOMAIN] | check HOST");
            2
        }
    }
}

fn cmd_policy(cfg: &Config, words: &[String], json: bool) -> u8 {
    let path = words
        .get(1)
        .map(std::path::PathBuf::from)
        .unwrap_or_else(policy::policy_path);
    match words.first().map(String::as_str).unwrap_or("check") {
        "rules" => {
        ...
        "check" | "audit" => {
            if !path.exists() {
                eprintln!(
                    "no policy at {}; write one with a rules object (phoenix policy rules lists them)",
                    path.display()
                );
                return 2;
            }
            let rules = match policy::load(&path) {
                Ok(r) => r,
                Err(e) => {
                    eprintln!("{e}");
                    return 2;
            ...
            let verdicts = policy::evaluate(cfg, &rules, &raw);
            print!("{}", policy::report_text(&verdicts, json));
            u8::from(verdicts.iter().any(|v| !v.pass))
        }
        other => {
            eprintln!("unknown policy subcommand '{other}': check [FILE] | rules");
            2
        }
    }
}

fn cmd_wiki(words: &[String]) -> u8 {
    let dir = wiki::vault_dir();
    match words.first().map(String::as_str).unwrap_or("status") {
        "status" | "list" => {
            print!("{}", wiki::status_text(&dir));
            0
        }
        "read" | "show" => {
            let Some(slug) = words.get(1) else {
                eprintln!("usage: phoenix wiki read SLUG");
                return 2;
            };
            match wiki::read(&dir, slug) {
                Ok(body) => {
                    print!("{}", crate::text::sanitize_terminal(&body));
                    0
                }
                Err(e) => {
                    eprintln!("{e}");
        ...
        "write" => {
            let Some(slug) = words.get(1) else {
                eprintln!("usage: phoenix wiki write SLUG BODY");
                return 2;
            };
            let body = words[2..].join(" ");
            match wiki::write(&dir, slug, &body) {
                Ok(path) => {
        ...
        "remove" | "delete" => {
            let Some(slug) = words.get(1) else {
                eprintln!("usage: phoenix wiki remove SLUG");
                return 2;
            };
            match wiki::remove(&dir, slug) {
                Ok(()) => {
        ...
        "search" => {
            let q = words[1..].join(" ");
            let hits = wiki::search(&dir, &q);
            if hits.is_empty() {
                println!("no pages match");
                return 0;
            }
            for (slug, line) in &hits {
                println!("  {slug:<24}{line}");
            }
            0
        }
        "lint" => {
            let broken = wiki::broken_links(&dir);
            let orphans = wiki::orphans(&dir);
            for b in &broken {
                println!("broken: {b}");
            }
            for o in &orphans {
            ...
            if broken.is_empty() && orphans.is_empty() {
                println!("the vault is clean");
            }
            u8::from(!broken.is_empty())
        }
        other => {
            eprintln!(
                "unknown wiki subcommand '{other}': status | read SLUG | write SLUG BODY | remove SLUG | search QUERY | lint"
            );
            2
        }
    }
...
fn cmd_fleet(words: &[String], force: bool) -> u8 {
    let root = fleet::root();
    let force = force || words.iter().any(|w| w == "--force");
    let words: Vec<&str> = words
        .iter()
        .map(String::as_str)
        .filter(|w| *w != "--force")
        .collect();
    match words.first().copied().unwrap_or("list") {
        "list" | "status" => {
            print!("{}", fleet::list_text(&fleet::list(&root)));
            0
        }
        "create" | "add" => {
            let Some(name) = words.get(1) else {
                eprintln!("usage: phoenix fleet create NAME");
                return 2;
            };
            match fleet::create(&root, name) {
                Ok(c) => {
                    println!("created cell {} at {}", c.name, c.dir.display());
                    println!("port {}, every door closed", c.port);
                    println!("use it with: eval \"$(phoenix fleet env {})\"", c.name);
                    0
                }
                Err(e) => {
                    eprintln!("{e}");
        ...
        "env" => {
            let Some(name) = words.get(1) else {
                eprintln!("usage: phoenix fleet env NAME");
                return 2;
            };
            match fleet::find(&root, name) {
                Some(c) => {
                    print!("{}", fleet::shell_text(&c));
                    0
                }
                None => {
                    eprintln!("no cell named '{name}'");
                    2
                }
            }
        }
        "remove" | "delete" => {
            let Some(name) = words.get(1) else {
                eprintln!("usage: phoenix fleet remove NAME --force");
                return 2;
            };
            match fleet::remove(&root, name, force) {
                Ok(dir) => {
        ...
        other => {
            eprintln!("unknown fleet subcommand '{other}': list | create NAME | env NAME | remove NAME --force");
            2
        }
    }
}

fn cmd_path(cfg: &Config, words: &[String]) -> u8 {
    match words.first().map(String::as_str).unwrap_or("help") {
        "resolve" | "read" => {
            let Some(addr) = words.get(1) else {
                eprintln!("usage: phoenix path resolve px://FILE/SEGMENT/...");
                return 2;
            };
            match path::resolve(&cfg.workspace, addr) {
                Ok(v) => {
                    println!("{}", crate::text::sanitize_terminal(&v));
                    0
                }
                Err(e) => {
                    eprintln!("{e}");
        ...
        "validate" => {
            let Some(addr) = words.get(1) else {
                eprintln!("usage: phoenix path validate px://FILE/SEGMENT/...");
                return 2;
            };
            match path::validate_text(addr) {
                Ok(t) => {
                    print!("{t}");
                    0
                }
        ...
        other => {
            eprintln!(
                "unknown path subcommand '{other}': resolve px://FILE/... | validate px://FILE/..."
            );
            2
        }
    }
...
fn cmd_claws(words: &[String]) -> u8 {
    let agents_dir = config::home().join("agents");
    let dir = words
        .get(1)
        .map(std::path::PathBuf::from)
        .unwrap_or_else(|| std::path::PathBuf::from("."));
    match words.first().map(String::as_str).unwrap_or("show") {
        "show" | "validate" => match claws::read(&dir) {
            Ok(c) => {
                print!("{}", claws::describe(&c));
                0
            }
            Err(e) => {
                eprintln!("{e}");
        ...
        "plan" => match claws::read(&dir) {
            Ok(c) => {
                for (i, step) in claws::plan(&c, &agents_dir).iter().enumerate() {
                    println!("  {}. {step}", i + 1);
                }
                println!("\nnothing was written; run `phoenix claws add DIR` to apply");
                0
            }
            Err(e) => {
                eprintln!("{e}");
        ...
        "add" | "install" => {
            match claws::read(&dir).and_then(|c| claws::install(&c, &agents_dir)) {
                Ok(path) => {
        ...
        other => {
            eprintln!("unknown claws subcommand '{other}': show [DIR] | plan [DIR] | add [DIR]");
            2
        }
    }
}
...
fn cmd_doctor(cfg: &Config, json: bool) -> u8 {
    ...
    if json {
        ...
        let tty = io::stdout().is_terminal();
        let ok = findings.iter().filter(|x| x.level == "ok").count();
        let warn = findings.iter().filter(|x| x.level == "warn").count();
        let fail = findings.iter().filter(|x| x.level == "fail").count();
        let lines: Vec<String> = findings
            .iter()
            .map(|x| format!("{} {}", text::status_tag(x.level, tty), x.msg))
            .collect();
        print!(
            "{}",
            text::draw_box(&format!("phoenix doctor v{VERSION}"), &lines, tty)
        );
        let summary = if fail > 0 {
            format!("{fail} failing, {warn} warning, {ok} clean")
        } else if warn > 0 {
            format!("{warn} warning, {ok} clean")
        } else {
            format!("all {ok} checks clean")
        };
        if tty {
            println!("{}{summary}{}", text::BOX_DIM, text::BOX_OFF);
        } else {
            println!("{summary}");
        }
    }
...
fn real_main() -> u8 {
    if let Err(error) = crate::daemon::install_handlers() {
    ...
    if first_flight {
        let gw = config::home_dir().join(".previous-gateway/gateway.json");
    ...
    match args.cmd {
        ...
        Cmd::Usage => return cmd_provider_usage(&cfg, args.json),
        ...
        Cmd::Board(words) => return cmd_board(&words, args.json),
        ...
        Cmd::Codex(words) => return cmd_codex(&words),
        Cmd::Transcribe(words) => return cmd_transcribe(&cfg, &words),
        Cmd::Worktrees(words) => return cmd_worktrees(&words),
        Cmd::Browser(words) => return cmd_browser(&cfg, &words),
        Cmd::Pairing(words) => return cmd_pairing(&cfg, &words),
        Cmd::Sandbox(words) => return cmd_sandbox(&cfg, &words),
        Cmd::Devices(words) => return cmd_devices(&words),
        Cmd::Qr(words) => return cmd_qr(&cfg, &words),
        Cmd::Nodes(words) => return cmd_nodes(&words),
        Cmd::Plugins(words) => return cmd_plugins(&cfg, &words, args.json),
        Cmd::Promos(words) => return cmd_promos(&cfg, &words),
        Cmd::Dns(words) => return cmd_dns(&cfg, &words),
        Cmd::Policy(words) => return cmd_policy(&cfg, &words, args.json),
        Cmd::Wiki(words) => return cmd_wiki(&words),
        Cmd::Fleet(words) => return cmd_fleet(&words, args.force),
        Cmd::PathAddr(words) => return cmd_path(&cfg, &words),
        Cmd::Claws(words) => return cmd_claws(&words),
    ...
    if cfg.api_key.is_empty()
        && cfg.provider != "ollama"
        && !providers::codex_active(&cfg)
        && args.provider.is_none()
        && args.model.is_none()
    {
        if let Some(found) = config::first_env_provider() {
            if found != cfg.provider {
                let was = cfg.provider.clone();
                config::switch_provider(&mut cfg, found);
                if let Some(m) = catalog::known_models(found).first() {
                    cfg.model = (*m).to_string();
                }
                eprintln!(
                    "no API key for {was}; found a {found} key in the environment, using {found}/{}",
                    cfg.model
                );
            }
        }
    ...
    if cfg.api_key.is_empty() && cfg.provider != "ollama" && !providers::codex_active(&cfg) {
...
or set one of {} in the environment",
    ...
    match args.cmd {
        ...
        Cmd::Usage => cmd_provider_usage(&cfg, args.json),
        Cmd::Schema
        | Cmd::Init
        | Cmd::Configure
        | Cmd::Status
...
fn cmd_status(json: bool) -> u8 {
    ...
    let key_in_store = {
        let store = secrets::Store::at(&secrets::Store::default_path());
        store.get("api_key").is_some()
            || store.get("PHOENIX_API_KEY").is_some()
            || config::provider_key_vars(&cfg.provider)
                .iter()
                .any(|v| store.get(v).is_some())
    };
    let key_source = if cfg.provider == "ollama" {
        "not needed"
    ...
    lines.push(format!("{dim}config   {off} {}", path.display()));
    lines.push(format!(
        "{dim}model    {off} {}/{}",
        cfg.provider, cfg.model
    ));
    if !cfg.fallbacks.is_empty() {
        lines.push(format!("{dim}fallbacks{off} {}", cfg.fallbacks.join(", ")));
    }
    let key_line_color = if tty && key_source == "MISSING" {
        text::BOX_FAIL
    } else {
        ""
    };
    lines.push(format!(
        "{dim}api key  {off} {key_line_color}{key_source}{off}"
    ));
    lines.push(format!("{dim}workspace{off} {}", cfg.workspace.display()));
    lines.push(format!(
        "{dim}colab    {off} off at start; per-chat toggle, /colab in chat shows live stats"
    ));
    lines.push(format!(
        "{dim}approvals{off} {}",
        if cfg.approvals {
    ...
    if channels.is_empty() {
        lines.push(format!("{dim}channels {off} none configured"));
    } else {
        let list: Vec<String> = channels
            ...
            .collect();
        lines.push(format!("{dim}channels {off} {}", list.join(", ")));
    }
    lines.push(format!("{dim}service  {off} {svc}"));
    if serve_running {
        let pid = serve["pid"].as_u64().unwrap_or(0);
        let up = scheduler::time_ago(serve["uptime_secs"].as_u64().unwrap_or(0));
        lines.push(match http_ms {
            Some(ms) => format!(
                "{dim}serve    {off} running as pid {pid} for {up}, /health answers in {ms} ms"
            ),
            None if cfg.http_enabled => format!(
                "{dim}serve    {off} running as pid {pid} for {up}, but /health is not answering"
            ),
            None => format!("{dim}serve    {off} running as pid {pid} for {up}"),
        });
    } else {
        lines.push(format!("{dim}serve    {off} not running"));
    }
    if sess.is_empty() {
        lines.push(format!("{dim}sessions {off} none stored"));
    } else {
        lines.push(format!(
    ...
    if cfg.http_enabled {
        ...
        if cfg.http_web {
            lines.push(format!("{dim}dashboard{off} {url}"));
        }
    } else {
        lines.push(format!("{dim}http     {off} off"));
    }
    match &upd {
        Some((at, tag, true)) => lines.push(format!(
            "{dim}update   {off} up to date with {tag} (checked {} ago)",
            scheduler::time_ago(now_secs.saturating_sub(*at))
        )),
        Some((at, tag, false)) => lines.push(format!(
            "{dim}update   {off} {tag} is out: run `phoenix update` (checked {} ago)",
            scheduler::time_ago(now_secs.saturating_sub(*at))
        )),
        None => lines.push(format!(
            "{dim}update   {off} never checked: run `phoenix update --check`"
        )),
    }
    print!(
        "{}",
        text::draw_box(&format!("openphoenix {VERSION}"), &lines, tty)
    );
...
fn cmd_provider_usage(cfg: &Config, json_out: bool) -> u8 {
    let snapshot = usage::fetch(cfg);
    if json_out {
        let windows: Vec<Value> = snapshot
            ...
            .collect();
        let billing: Vec<Value> = snapshot
            .billing
            .iter()
            .map(|entry| {
                json!({
        ...
        println!("{}", snapshot.short());
        match snapshot.state() {
            usage::QuotaState::Low => println!(
                "Recommendation: keep the next turn focused or choose a provider with more room. Your model choice stays unchanged."
            ),
            usage::QuotaState::Exhausted => println!(
                "Recommendation: wait for reset or choose /model and /models alternatives. Phoenix never changes your explicit choice silently."
            ),
            usage::QuotaState::Ready | usage::QuotaState::Unknown => {}
        }
    }
    0
}
...
fn cmd_migrate(from: Option<&str>, write: bool, force: bool, secrets: bool) -> u8 {
    let src = from
        .map(std::path::PathBuf::from)
        .unwrap_or_else(|| crate::config::home_dir().join(".previous-gateway/gateway.json"));
    ...
    if write && !m.secrets.is_empty() {
        let carried: Vec<(String, Vec<String>)> = m
            .secrets
            .iter()
            .map(|(n, s)| (n.clone(), vec![s.clone()]))
            .collect();
        if let Err(e) = secrets::stash_named(&carried) {
            eprintln!("error: cannot encrypt the carried token: {e}");
            return 2;
        }
    }
    if !keys.is_empty() {
        if let Err(e) = secrets::stash_provider_keys(&keys) {
            eprintln!("error: cannot encrypt the carried keys: {e}");
            return 2;
        }
    ...
    if write {
        ...
        println!("wrote {}", path.display());
        if let Some(t) = oauth::import_from_claude_cli() {
            match oauth::save(&t) {
                Ok(()) => println!(
                    "carried Anthropic OAuth into {} (auto-refreshing)",
                    oauth::store_path().display()
                ),
                Err(e) => eprintln!("found Anthropic OAuth but could not save it: {e}"),
            }
        }
        if let Some(dir) = src.parent() {
...
fn install_panic_hook() {
    std::panic::set_hook(Box::new(|info| {
        ...
        crate::log::error(
            "panic",
            format!(
                "internal error; shutting down cleanly: {} (at {at})",
...
mod tests {
    ...
    fn stop_guard() -> std::sync::MutexGuard<'static, ()> {
    ...
    #[test]
    fn stop_raises_the_interrupt_even_with_no_pending_approvals() {
        let _g = stop_guard();
        let cfg = Config::default();
        let reply = channel_command(None, &cfg, "/stop").expect("/stop is a command");
        assert!(
            crate::daemon::interrupt_pending_swap(),
            "/stop must interrupt the running turn, not only deny queued approvals: {}",
            reply.text
        );
    }

    #[test]
    fn a_plain_stop_also_interrupts_the_running_turn() {
        let _g = stop_guard();
        let cfg = Config::default();
        for word in ["stop", "stop please", "abort", "cancel"] {
            crate::daemon::set_interrupt_pending(false);
            let reply = channel_command(None, &cfg, word).unwrap_or_else(|| {
                panic!("{word:?} is an abort request");
            });
            assert!(
                crate::daemon::interrupt_pending_swap(),
                "{word:?} must interrupt the turn: {}",
                reply.text
            );
        }
    }
    ...
    #[test]
    fn every_channel_arms_the_interrupt_around_a_turn() {
        let src = std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/src/main.rs"))
            .expect("main.rs is readable");
        let bad = format!(".map(|a| a.{}(text))", "run");
        assert!(
            !src.contains(&bad),
            "a channel turn must go through run_turn so the thread arms the interrupt; \
 an unarmed thread ignores /stop entirely"
        );
    }

    #[test]
    fn a_subcommand_namespace_keeps_its_own_status_word() {
        let args = |v: &[&str]| v.iter().map(|s| s.to_string()).collect::<Vec<_>>();
        let parsed = parse_args(&args(&["service", "status"])).expect("parses");
        assert!(
            matches!(&parsed.cmd, Cmd::Service(w) if w == &["status".to_string()]),
            "`phoenix service status` must reach the service namespace, not global status"
        );
    }
...
mod legacy_tests {
    ...
    fn model_alias_switch_retargets_the_provider_too() {
        ...
        assert!(r.text.contains("anthropic/claude-opus-5"), "{}", r.text);
    ...
    fn silent_agent(provider: &str, model: &str) -> (Agent, Config) {
        struct Silent;
        impl providers::ChatBackend for Silent {
            fn chat(
                &mut self,
        ...
        let _ = std::fs::create_dir_all(&cfg.workspace);
        let toolbox = Toolbox::new(&cfg, Memory::new("ghost"), None, None).unwrap();
        let a = Agent::new(cfg.clone(), Box::new(Silent), toolbox);
        (a, cfg)
    }

    #[test]
    fn model_with_no_arg_shows_a_provider_picker_first() {
        let cfg = Config {
            provider: "anthropic".into(),
            ..Config::default()
        };
        let r = channel_command(None, &cfg, "/model").expect("handled");
        assert!(!r.buttons.is_empty(), "expected provider buttons");
        let callbacks: Vec<String> = r
            .buttons
            .iter()
            .flatten()
            .map(|(_, cb)| cb.clone())
            .collect();
        assert!(
            callbacks.iter().any(|c| c == "/model openai"),
            "provider picker must offer openai: {callbacks:?}"
        );
        assert!(
            callbacks
                .iter()
                .all(|c| !c.contains('/') || c.starts_with("/model ")),
            "step-1 callbacks are provider-only: {callbacks:?}"
        );
        let marked: Vec<String> = r.buttons.iter().flatten().map(|(l, _)| l.clone()).collect();
        assert!(
            marked
                .iter()
                .any(|l| l.contains("✅") && l.contains("anthropic")),
            "current provider highlighted: {marked:?}"
        );
    }

    #[test]
    fn model_with_a_known_provider_shows_that_providers_models() {
        let cfg = Config {
            provider: "openai".into(),
            ..Config::default()
        };
        let r = channel_command(None, &cfg, "/model nvidia").expect("handled");
        assert!(!r.buttons.is_empty(), "expected nvidia model buttons");
        let callbacks: Vec<String> = r
            .buttons
            .iter()
            .flatten()
        ...
        assert!(
            callbacks
                .iter()
                .any(|c| c.starts_with("/model_exact nvidia/")),
            "step-2 callbacks switch both provider and model: {callbacks:?}"
        );
        assert!(
            callbacks.iter().all(|c| {
                c.starts_with("/model_exact nvidia/") || c.starts_with("/model nvidia page=")
            }),
            "model and navigation callbacks stay in the provider: {callbacks:?}"
        );
    }

    #[test]
    fn model_pages_are_bounded_complete_and_navigable() {
        let names: Vec<String> = (1..=23).map(|i| format!("model-{i:02}")).collect();
        let first = model_page(
            &names,
            1,
            "",
            |page| format!("/model test page={page}"),
            |name| format!("/model test/{name}"),
        );
        assert_eq!(
            (first.page, first.total_pages, first.total_items),
            (1, 3, 23)
        );
        assert_eq!(
            first
                ...
                .iter()
                .flatten()
                .filter(|(_, callback)| callback.starts_with("/model test/model-"))
                .count(),
            10
        );
        let first_callbacks: Vec<&str> = first
            .rows
            .iter()
            .flatten()
            .map(|(_, callback)| callback.as_str())
            .collect();
        assert!(!first_callbacks
            .iter()
            .any(|callback| callback.contains("page=0")));
        assert!(first_callbacks.contains(&"/model test page=2"));
        ...
        assert_eq!(last.page, 3, "out-of-range pages clamp to the last page");
        let last_callbacks: Vec<&str> = last
            .rows
            .iter()
            .flatten()
            .map(|(_, callback)| callback.as_str())
            .collect();
        assert!(last_callbacks.contains(&"/model test/model-23"));
        assert!(last_callbacks.contains(&"/model test page=2"));
        assert!(!last_callbacks
            .iter()
            .any(|callback| callback.contains("page=4")));
    }

    #[test]
    fn long_model_ids_use_a_short_button_token_and_resolve_to_the_raw_action() {
        let names = vec![format!("vendor/{}", "long-model-segment-".repeat(4))];
        let page = model_page(
            &names,
            1,
            "",
            |number| format!("/model test page={number}"),
            |name| format!("/model test/{name}"),
        );
        let wire = &page.rows[0][0].1;
        assert!(wire.starts_with("/pick "), "{wire}");
        assert!(wire.len() <= telegram::CALLBACK_DATA_MAX_BYTES);
        assert_eq!(page.actions.len(), 1);
        assert_eq!(page.actions[0].0, *wire);
        assert_eq!(page.actions[0].1, format!("/model test/{}", names[0]));
    }

    #[test]
    fn short_callbacks_that_fail_telegram_validation_use_an_opaque_token() {
        let names = vec!["model+with+unsupported+characters".to_string()];
        let page = model_page(
            &names,
            1,
            "",
            |number| format!("/model test page={number}"),
            |name| format!("/model_exact test/{name}"),
        );
        let wire = &page.rows[0][0].1;
        assert!(wire.starts_with("/pick "), "{wire}");
        assert!(telegram::valid_callback(wire));
        assert_eq!(
            page.actions[0].1,
            "/model_exact test/model+with+unsupported+characters"
        );
    }

    #[test]
    fn exact_picker_choice_preserves_namespaced_model_ids() {
        let (mut a, cfg) = silent_agent("nvidia", "z-ai/glm-5.2");
        let reply = channel_command(
            Some(&mut a),
            &cfg,
            "/model_exact nvidia/nvidia/nemotron-3-super-120b-a12b",
        )
        .expect("handled");
        assert!(reply
            .text
            .contains("nvidia/nvidia/nemotron-3-super-120b-a12b"));
        assert_eq!(a.cfg.provider, "nvidia");
        assert_eq!(a.cfg.model, "nvidia/nemotron-3-super-120b-a12b");
    }

    #[test]
    fn a_model_is_marked_only_under_its_current_provider() {
        let names = vec!["shared-id".to_string()];
        let current = model_page(
            &names,
            1,
            "shared-id",
            |_| "/model openai page=1".to_string(),
            |name| format!("/model_exact openai/{name}"),
        );
        let other = model_page(
            &names,
            1,
            "",
            |_| "/model custom page=1".to_string(),
            |name| format!("/model_exact custom/{name}"),
        );
        assert!(current.rows[0][0].0.starts_with("✅ "));
        assert!(!other.rows[0][0].0.starts_with("✅ "));
    }

    #[test]
    fn openai_picker_shows_friendly_names_raw_ids_and_page_controls() {
        let cfg = Config {
            provider: "openai".into(),
            ..Config::default()
        };
        let first = channel_command(None, &cfg, "/model openai").expect("handled");
        assert!(first.text.contains("page 1/2"), "{}", first.text);
        ...
        assert!(
            labels
                .iter()
                .any(|label| label.contains("GPT-5.4") && label.contains("gpt-5.4")),
            "friendly name and raw id must both stay visible: {labels:?}"
        );
        let callbacks: Vec<&str> = first
            .buttons
            .iter()
            .flatten()
            .map(|(_, callback)| callback.as_str())
            .collect();
        assert!(callbacks.contains(&"/model openai page=2"), "{callbacks:?}");

        let second = channel_command(None, &cfg, "/model openai page=2").expect("handled");
        assert!(second.text.contains("page 2/2"), "{}", second.text);
        let second_callbacks: Vec<&str> = second
            .buttons
            .iter()
            .flatten()
            .map(|(_, callback)| callback.as_str())
            .collect();
        assert!(second_callbacks.contains(&"/model openai page=1"));
        assert!(second_callbacks
            .iter()
            .any(|callback| callback == &"/model_exact openai/o4-mini"));
    }

    #[test]
    fn numbered_model_pages_include_prev_page_next_as_choices() {
        let cfg = Config {
            provider: "openai".into(),
            ..Config::default()
        };
        let reply = channel_command(None, &cfg, "/model openai page=2").expect("handled");
        let (shown, pairs) = render_numbered(&reply);
        assert!(shown.contains("Prev"), "{shown}");
        assert!(shown.contains("2/2"), "{shown}");
        let prev = pairs
            .iter()
            .position(|(label, _)| label.contains("Prev"))
            .expect("prev choice");
        assert_eq!(
            resolve_pick(&pairs, &(prev + 1).to_string()).as_deref(),
            Some("/model openai page=1")
        );
    }

    #[test]
    fn models_no_arg_opens_the_colab_menu_with_colab_off() {
        let cfg = Config::default();
        let r = channel_command(None, &cfg, "/models").expect("handled");
        assert!(r.text.contains("Colab: off"), "{}", r.text);
        let callbacks: Vec<String> = r
            .buttons
            .iter()
            .flatten()
            ...
            .collect();
        assert!(
            callbacks.contains(&"/models main".to_string()),
            "{callbacks:?}"
        );
        assert!(
            callbacks.contains(&"/models partner".to_string()),
            "{callbacks:?}"
        );
        assert!(
            callbacks.contains(&"/colab auto".to_string()),
            "{callbacks:?}"
        );
    }

    #[test]
    fn models_menu_with_colab_on_offers_both_models_and_off() {
        let (mut a, cfg) = silent_agent("openai", "gpt-5.4");
        channel_command(Some(&mut a), &cfg, "/colab on ollama/llama3").expect("handled");
        let r = channel_command(Some(&mut a), &cfg, "/models").expect("handled");
        assert!(r.text.contains("Colab: ON"), "{}", r.text);
        assert!(
            r.text.contains("🪶 Partner model: ollama/llama3"),
            "{}",
            r.text
        );
        let callbacks: Vec<String> = r
        ...
        assert!(
            callbacks.contains(&"/colab off".to_string()),
            "{callbacks:?}"
        );
    }

    #[test]
    fn models_partner_step_lists_providers_with_partner_callbacks() {
        let cfg = Config::default();
        let r = channel_command(None, &cfg, "/models partner").expect("handled");
        assert!(!r.buttons.is_empty());
        assert!(
            r.buttons
                .iter()
                .flatten()
                .all(|(_, cb)| cb.starts_with("/models partner ")),
            "partner step must stay in the partner flow"
        );
    }

    #[test]
    fn picking_a_partner_model_emits_a_colab_on_callback() {
        let cfg = Config {
            provider: "openai".into(),
            model: "gpt-5.4".into(),
            ..Config::default()
        };
        let r = channel_command(None, &cfg, "/models partner nvidia").expect("handled");
        assert!(!r.buttons.is_empty(), "{}", r.text);
        let callbacks: Vec<&str> = r
            .buttons
            .iter()
            .flatten()
            .map(|(_, callback)| callback.as_str())
        ...
        assert!(
            callbacks
                .iter()
                .any(|callback| callback.starts_with("/colab on nvidia/")),
            "picking a partner model must enable colab"
        );
        assert!(
            callbacks.iter().all(|callback| {
                callback.starts_with("/colab on nvidia/")
                    || callback.starts_with("/models partner nvidia page=")
                    || callback.starts_with("/pick ")
            }),
            "partner navigation must stay in the same flow: {callbacks:?}"
        );
        assert!(r.text.contains("turns colab on"), "{}", r.text);
    }

    #[test]
    fn models_off_delegates_to_colab_off() {
        let (mut a, cfg) = silent_agent("openai", "gpt-5.4");
        let r = channel_command(Some(&mut a), &cfg, "/models off").expect("handled");
        assert!(r.text.contains("already off"), "{}", r.text);
        channel_command(Some(&mut a), &cfg, "/colab on ollama/llama3").expect("handled");
        let r = channel_command(Some(&mut a), &cfg, "/models off").expect("handled");
        assert!(r.text.contains("now off"), "{}", r.text);
        assert!(!a.colab_on());
    }

    #[test]
    fn reasoning_visibility_is_separate_from_thinking() {
        let (mut agent, cfg) = silent_agent("openai", "gpt-5.4");
        assert!(!agent.cfg.reasoning_visible);
        let on = channel_command(Some(&mut agent), &cfg, "/reason on").expect("handled");
        assert!(
            on.text.contains("detailed public team work log"),
            "{}",
            on.text
        );
        assert!(agent.cfg.reasoning_visible);
        assert_eq!(agent.cfg.thinking, cfg.thinking);
        channel_command(Some(&mut agent), &cfg, "/reasoning off").expect("handled");
        assert!(!agent.cfg.reasoning_visible);
    }

    #[test]
    fn think_picker_only_offers_levels_the_model_supports() {
        let (mut plain, cfg) = silent_agent("openai", "gpt-4o");
        let r = channel_command(Some(&mut plain), &cfg, "/think").expect("handled");
        let labels: Vec<String> = r
            .buttons
            .iter()
            .flatten()
            .map(|(l, _)| l.trim_start_matches("\u{2705} ").to_string())
            .collect();
        assert_eq!(labels, vec!["default", "off"], "{labels:?}");
        assert!(r.text.contains("no adjustable thinking"), "{}", r.text);

        let (mut reasoning, cfg) = silent_agent("openai", "gpt-5.4");
        let r = channel_command(Some(&mut reasoning), &cfg, "/think").expect("handled");
        let labels: Vec<String> = r
            .buttons
            .iter()
            .flatten()
            .map(|(l, _)| l.trim_start_matches("\u{2705} ").to_string())
            .collect();
        assert!(labels.contains(&"high".to_string()), "{labels:?}");
        assert!(!labels.contains(&"xhigh".to_string()), "{labels:?}");

        let (mut claude, cfg) = silent_agent("anthropic", "claude-sonnet-5");
        let r = channel_command(Some(&mut claude), &cfg, "/think").expect("handled");
        let labels: Vec<String> = r
            .buttons
            .iter()
            .flatten()
            .map(|(l, _)| l.trim_start_matches("\u{2705} ").to_string())
            .collect();
        assert!(labels.contains(&"max".to_string()), "{labels:?}");
    }

    #[test]
    fn setting_an_unsupported_think_level_warns_but_sticks() {
        let (mut a, cfg) = silent_agent("openai", "gpt-4o");
        let r = channel_command(Some(&mut a), &cfg, "/think high").expect("handled");
        assert_eq!(a.cfg.thinking, "high");
        assert!(r.text.contains("no adjustable thinking"), "{}", r.text);
    }

    #[test]
    fn think_sets_the_whole_team_and_can_target_the_partner() {
        let (mut a, cfg) = silent_agent("anthropic", "claude-sonnet-5");
        channel_command(Some(&mut a), &cfg, "/colab on ollama/llama3").expect("handled");
        let r = channel_command(Some(&mut a), &cfg, "/think high").expect("handled");
        assert!(r.text.contains("team"), "{}", r.text);
        assert!(
            r.text.contains("main anthropic/claude-sonnet-5"),
            "{}",
            r.text
        );
        assert!(r.text.contains("partner ollama/llama3"), "{}", r.text);
        assert_eq!(a.cfg.thinking, "high");
        let r = channel_command(Some(&mut a), &cfg, "/think partner low").expect("handled");
        assert!(r.text.contains("partner thinking"), "{}", r.text);
        assert!(r.text.contains("ollama/llama3"), "{}", r.text);
    }

    #[test]
    fn think_offers_a_per_model_chooser_and_both() {
        let (mut a, cfg) = silent_agent("anthropic", "claude-sonnet-5");
        channel_command(Some(&mut a), &cfg, "/colab on ollama/llama3").expect("handled");
        let r = channel_command(Some(&mut a), &cfg, "/think").expect("handled");
        let cbs: Vec<String> = r.buttons.iter().flatten().map(|(_, c)| c.clone()).collect();
        assert!(cbs.contains(&"/think main".to_string()), "{cbs:?}");
        assert!(cbs.contains(&"/think partner".to_string()), "{cbs:?}");
        assert!(cbs.contains(&"/think both".to_string()), "{cbs:?}");
        let r = channel_command(Some(&mut a), &cfg, "/think both medium").expect("handled");
        assert!(r.text.contains("team"), "{}", r.text);
        assert_eq!(a.cfg.thinking, "medium");
        let r = channel_command(Some(&mut a), &cfg, "/think main").expect("handled");
        let cbs: Vec<String> = r.buttons.iter().flatten().map(|(_, c)| c.clone()).collect();
        assert!(cbs.iter().all(|c| c.starts_with("/think main ")), "{cbs:?}");
    }

    #[test]
    fn direct_provider_slash_model_spec_still_switches() {
        let (mut a, cfg) = silent_agent("openai", "gpt-5.4");
        let r = channel_command(Some(&mut a), &cfg, "/model anthropic/claude-sonnet-5")
            .expect("handled");
        assert!(r.text.contains("anthropic/claude-sonnet-5"), "{}", r.text);
        assert_eq!(a.cfg.provider, "anthropic");
        assert_eq!(a.cfg.model, "claude-sonnet-5");
    }

    #[test]
    fn colab_status_is_off_by_default() {
        let (mut a, cfg) = silent_agent("openai", "gpt-5.4");
        assert!(!a.colab_on());
        let r = channel_command(Some(&mut a), &cfg, "/colab").expect("handled");
        assert!(r.text.contains("off"), "{}", r.text);
    }

    #[test]
    fn colab_off_when_already_off_says_so() {
        let (mut a, cfg) = silent_agent("openai", "gpt-5.4");
        let r = channel_command(Some(&mut a), &cfg, "/colab off").expect("handled");
        assert!(r.text.contains("already off"), "{}", r.text);
    }

    #[test]
    fn colab_toggle_on_with_ollama_then_off() {
        let (mut a, cfg) = silent_agent("openai", "gpt-5.4");
        let r = channel_command(Some(&mut a), &cfg, "/colab on ollama/llama3").expect("handled");
        assert!(r.text.contains("Colab is ON"), "{}", r.text);
        assert!(a.colab_on());
        assert_eq!(
            a.colab.as_ref().map(|c| c.partner.as_str()),
            Some("ollama/llama3")
        );
        let r = channel_command(Some(&mut a), &cfg, "/colab").expect("handled");
        assert!(
    ...
    #[test]
    fn colab_on_with_rounds_flag_is_parsed() {
        let (mut a, cfg) = silent_agent("openai", "gpt-5.4");
        let r = channel_command(Some(&mut a), &cfg, "/colab on ollama/llama3 --rounds 4")
            .expect("handled");
        assert!(r.text.contains("Colab is ON"), "{}", r.text);
        assert_eq!(a.colab.as_ref().map(|c| c.max_rounds), Some(4));
    }

    #[test]
    fn colab_refuses_the_current_model_as_partner() {
        let (mut a, cfg) = silent_agent("openai", "gpt-5.4");
        let r = channel_command(Some(&mut a), &cfg, "/colab on openai/gpt-5.4").expect("handled");
        assert!(r.text.contains("not enabled"), "{}", r.text);
        assert!(!a.colab_on());
    }

    #[test]
    fn colab_refuses_an_unknown_provider() {
        let (mut a, cfg) = silent_agent("openai", "gpt-5.4");
        let r = channel_command(Some(&mut a), &cfg, "/colab on bogusprov/x").expect("handled");
        assert!(r.text.contains("not enabled"), "{}", r.text);
        assert!(r.text.contains("does not know provider"), "{}", r.text);
    }

    #[test]
    fn colab_auto_returns_a_handled_reply() {
        let (mut a, cfg) = silent_agent("openai", "gpt-5.4");
        let r = channel_command(Some(&mut a), &cfg, "/colab auto").expect("handled");
        assert!(
            r.text.contains("Colab is ON") || r.text.contains("no other provider"),
            "{}",
            r.text
        );
    }
    ...
    #[test]
    fn switching_providers_preserves_history_and_survives_a_missing_key() {
        let (mut a, cfg) = silent_agent("openai", "gpt-5.4");
        a.history.push(providers::Msg::User {
            content: "remember me".into(),
            images: Vec::new(),
        });
        a.history.push(providers::Msg::Assistant {
            content: "noted".into(),
            tool_calls: Vec::new(),
        });
        let r = channel_command(Some(&mut a), &cfg, "/model byteplus/seed-1").expect("handled");
        assert!(r.text.contains("byteplus/seed-1"), "{}", r.text);
        assert_eq!(a.cfg.provider, "byteplus");
        assert_eq!(
            a.history.len(),
            2,
            "switching providers must not wipe session history"
        );
        assert!(
            matches!(&a.history[0], providers::Msg::User { content, .. } if content == "remember me")
        );
        if a.cfg.api_key.is_empty() {
            assert!(
                r.text.contains("no API key"),
                "keyless switch must warn: {}",
                r.text
            );
        }
    }
    ...
    #[test]
    fn an_unknown_provider_prefix_stays_on_the_current_provider() {
        let (mut a, cfg) = silent_agent("openai", "gpt-5.4");
        let r = channel_command(Some(&mut a), &cfg, "/model bogusprov/x").expect("handled");
        assert_eq!(a.cfg.provider, "openai");
        assert_eq!(a.cfg.model, "bogusprov/x");
        assert!(r.text.contains("openai/bogusprov/x"), "{}", r.text);
        let r = channel_command(Some(&mut a), &cfg, "/model totallyfake").expect("handled");
        assert_eq!(a.cfg.provider, "openai");
        assert_eq!(a.cfg.model, "totallyfake");
        assert!(r.text.contains("openai/totallyfake"), "{}", r.text);
    }

    #[test]
    fn models_provider_flag_selects_the_provider_without_a_config_edit() {
        let a =
            parse_args(&["models".into(), "--provider".into(), "nvidia".into()]).expect("parsed");
        assert_eq!(a.cmd, Cmd::Models);
        assert_eq!(a.provider.as_deref(), Some("nvidia"));
        let mut cfg = Config::default();
        config::switch_provider(&mut cfg, "nvidia");
        assert_eq!(cfg.provider, "nvidia");
        assert!(
            !catalog::known_models("nvidia").is_empty(),
            "offline catalogue must know nvidia models for the keyless list"
        );
    }

    fn picker_reply() -> CmdReply {
        CmdReply {
            text: "pick a provider:".into(),
            buttons: vec![vec![
                ("\u{2705} anthropic".into(), "/model anthropic".into()),
                ("openai".into(), "/model openai".into()),
            ]],
            actions: Vec::new(),
        }
    }
    ...
    #[test]
    fn the_cli_picker_prints_a_numbered_list_and_accepts_a_number() {
        let reply = picker_reply();
        let mut input = std::io::Cursor::new(b"2\n".to_vec());
        let mut out: Vec<u8> = Vec::new();
        let picked = cli_pick(&reply, &mut input, &mut out);
        assert_eq!(picked.as_deref(), Some("/model openai"));
        let shown = String::from_utf8(out).unwrap();
        assert!(shown.contains("1) anthropic"), "{shown}");
        assert!(shown.contains("2) openai"), "{shown}");
    }

    #[test]
    fn the_cli_picker_accepts_a_name_and_rejects_garbage() {
        let reply = picker_reply();
        let mut out: Vec<u8> = Vec::new();
        let by_name = cli_pick(
            &reply,
            &mut std::io::Cursor::new(b"OPENAI\n".to_vec()),
            &mut out,
        );
        assert_eq!(by_name.as_deref(), Some("/model openai"));
        let by_prefix = cli_pick(
            &reply,
            &mut std::io::Cursor::new(b"anthro\n".to_vec()),
            &mut out,
        );
        assert_eq!(by_prefix.as_deref(), Some("/model anthropic"));
        assert_eq!(
            cli_pick(
                &reply,
                &mut std::io::Cursor::new(b"zzz\n".to_vec()),
                &mut out
            ),
            None
        );
        assert_eq!(
            cli_pick(
                &reply,
                &mut std::io::Cursor::new(b"99\n".to_vec()),
                &mut out
            ),
            None
        );
        assert_eq!(
            cli_pick(&reply, &mut std::io::Cursor::new(b"\n".to_vec()), &mut out),
            None
        );
    }

    #[test]
    fn resolve_pick_matches_number_name_and_unique_prefix() {
        let pairs = vec![
            ("anthropic".to_string(), "/model anthropic".to_string()),
            ("openai".to_string(), "/model openai".to_string()),
            ("openrouter".to_string(), "/model openrouter".to_string()),
            ("🔑 nvidia".to_string(), "/model nvidia".to_string()),
        ];
        assert_eq!(
            resolve_pick(&pairs, "nvidia").as_deref(),
            Some("/model nvidia"),
            "a key marker on the label must not break name matching"
        );
        assert_eq!(
            resolve_pick(&pairs, "nvid").as_deref(),
            Some("/model nvidia")
        );
        assert_eq!(
            resolve_pick(&pairs, "1").as_deref(),
            Some("/model anthropic")
        );
        assert_eq!(
            resolve_pick(&pairs, "ANTHROPIC").as_deref(),
            Some("/model anthropic")
        );
        assert_eq!(
            resolve_pick(&pairs, "anthro").as_deref(),
            Some("/model anthropic")
        );
        assert_eq!(
            resolve_pick(&pairs, "open"),
            None,
            "ambiguous prefix picks nothing"
        );
        assert_eq!(resolve_pick(&pairs, "9"), None);
        assert_eq!(resolve_pick(&pairs, ""), None);
    }

    #[test]
    fn render_numbered_produces_a_reply_with_a_number_prompt() {
        let reply = CmdReply {
            text: "Select a provider:".into(),
            buttons: vec![vec![
                ("\u{2705} anthropic".into(), "/model anthropic".into()),
                ("openai".into(), "/model openai".into()),
            ]],
        ...
        let (text, pairs) = render_numbered(&reply);
        assert!(text.contains("Select a provider:"), "{text}");
        assert!(text.contains(" 1. anthropic"), "{text}");
        assert!(text.contains(" 2. openai"), "{text}");
        assert!(text.contains("Reply with a number"), "{text}");
        assert_eq!(pairs.len(), 2);
        assert_eq!(
            pairs[0].0, "anthropic",
            "checkmark stripped from stored label"
        );
    }

    #[test]
    fn a_text_channel_picker_resolves_a_numeric_reply_across_two_steps() {
        let (mut a, cfg) = silent_agent("openai", "gpt-5.4");
        let first = text_channel_reply(Some(&mut a), &cfg, "/think").expect("handled");
        assert!(first.contains("Reply with a number"), "{first}");
        assert!(a.pending_pick.is_some(), "picker remembered for the reply");
        let second = text_channel_reply(Some(&mut a), &cfg, "2").expect("handled");
        assert_eq!(a.cfg.thinking, config::THINKING_LEVELS[1], "{second}");
        assert!(
            a.pending_pick.is_none(),
            "a resolved leaf choice clears the pending picker"
        );
    }

    #[test]
    fn a_text_channel_reply_ignores_a_stale_number_when_nothing_is_pending() {
        let (mut a, cfg) = silent_agent("openai", "gpt-5.4");
        assert!(a.pending_pick.is_none());
        assert!(
            text_channel_reply(Some(&mut a), &cfg, "2").is_none(),
            "a bare number with no pending picker is just a normal message"
        );
    }
    ...
    #[test]
    fn reason_aliases_offer_safe_visibility_toggle() {
        let (mut agent, cfg) = silent_agent("openai", "gpt-5.4");
        for command in ["/reason", "/reasoning"] {
            let reply = channel_command(Some(&mut agent), &cfg, command).expect("handled");
            let actions: Vec<&str> = reply
                .buttons
                .iter()
                .flatten()
                .map(|(_, action)| action.as_str())
                .collect();
            assert!(actions.contains(&"/reasoning on"), "{command}: {actions:?}");
            assert!(reply.text.contains("never private chain-of-thought"));
        }
    }

    #[test]
    fn think_alias_has_supported_levels() {
        let cfg = Config::default();
        let expected: Vec<String> = providers::thinking_levels_for(&cfg)
            .iter()
            .map(|s| s.to_string())
            .collect();
        for cmd in ["/think", "/thinking"] {
            ...
            let labels: Vec<String> = r
                ...
                .map(|(l, _)| l.replace("\u{2705} ", ""))
                .collect();
            assert_eq!(labels, expected, "{cmd}");
            assert!(labels.contains(&"default".to_string()), "{cmd}: {labels:?}");
            assert!(labels.contains(&"off".to_string()), "{cmd}: {labels:?}");
        }
    }
    ...
    #[test]
    fn flatten_renders_a_numbered_list_for_buttonless_channels() {
        let cfg = Config::default();
        let r = channel_command(None, &cfg, "/lean").expect("handled");
        let flat = r.flatten();
        assert!(flat.contains(" 1. off"), "{flat}");
        assert!(flat.contains(" 2. lean"), "{flat}");
        assert!(flat.contains(" 3. grunt"), "{flat}");
        assert!(flat.contains("Reply with a number"), "{flat}");
    ...
    #[test]
    fn every_new_command_parses_into_its_own_arm() {
        type Check = fn(&Cmd) -> bool;
        let cases: Vec<(&str, Check)> = vec![
            ("browser", |c| matches!(c, Cmd::Browser(_))),
            ("pairing", |c| matches!(c, Cmd::Pairing(_))),
            ("sandbox", |c| matches!(c, Cmd::Sandbox(_))),
            ("devices", |c| matches!(c, Cmd::Devices(_))),
            ("qr", |c| matches!(c, Cmd::Qr(_))),
            ("nodes", |c| matches!(c, Cmd::Nodes(_))),
            ("node", |c| matches!(c, Cmd::Nodes(_))),
            ("plugins", |c| matches!(c, Cmd::Plugins(_))),
            ("promos", |c| matches!(c, Cmd::Promos(_))),
            ("dns", |c| matches!(c, Cmd::Dns(_))),
            ("policy", |c| matches!(c, Cmd::Policy(_))),
            ("wiki", |c| matches!(c, Cmd::Wiki(_))),
            ("fleet", |c| matches!(c, Cmd::Fleet(_))),
            ("path", |c| matches!(c, Cmd::PathAddr(_))),
            ("claws", |c| matches!(c, Cmd::Claws(_))),
            ("acp", |c| matches!(c, Cmd::Acp)),
            ("workboard", |c| matches!(c, Cmd::Board(_))),
            ("flows", |c| matches!(c, Cmd::Tasks(_))),
        ];
        for (name, check) in cases {
            let a = parse_args(&[name.to_string()])
                .unwrap_or_else(|e| panic!("{name} must parse, got: {e}"));
            assert!(check(&a.cmd), "{name} landed in the wrong arm: {:?}", a.cmd);
        }
    }

    #[test]
    fn subcommand_words_reach_the_new_arms_intact() {
        let a = parse_args(&["pairing".into(), "approve".into(), "AB12CD".into()]).unwrap();
        assert_eq!(a.cmd, Cmd::Pairing(vec!["approve".into(), "AB12CD".into()]));
        let b = parse_args(&["path".into(), "resolve".into(), "px://a.json/b".into()]).unwrap();
        assert_eq!(
            b.cmd,
            Cmd::PathAddr(vec!["resolve".into(), "px://a.json/b".into()])
        );
    }
