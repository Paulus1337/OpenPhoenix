...
pub fn install(cfg: &Config, reference: &str, skills_dir: &Path) -> Result<String, String> {
    ...
    let digest = v["archive"]["sha256"].as_str();
    verify_archive(&bytes, digest)?;
    let integrity = if digest.is_some() {
        ", checksum verified"
    } else {
        ""
    };
    ...
    Ok(format!(
        "installed {reference} {version} into {}{integrity}\nnote: third-party skill, review {} before relying on it",
        target.display(),
        target.join("SKILL.md").display()
    ))
}

pub fn verify_archive(bytes: &[u8], expected_sha256: Option<&str>) -> Result<(), String> {
    ...
    let got = crate::security::sha256_hex(bytes);
    if got == expected {
        Ok(())
    } else {
        Err(format!(
            "archive checksum mismatch: registry says {expected}, download is {got}; refusing"
        ))
    }
}
...
mod tests {
    ...
    #[test]
    fn a_registry_digest_gates_the_install() {
        let bytes = b"---\nname: x\n---\nbody";
        let good = crate::security::sha256_hex(bytes);
        assert!(verify_archive(bytes, Some(&good)).is_ok());
        assert!(
            verify_archive(bytes, Some(&good.to_uppercase())).is_ok(),
            "digest comparison is case-insensitive"
        );
        let err = verify_archive(bytes, Some("deadbeef")).unwrap_err();
        assert!(err.contains("checksum mismatch"), "{err}");
        assert!(
            verify_archive(bytes, None).is_ok(),
            "a registry without digests still installs, with no verified claim"
        );
        assert!(verify_archive(bytes, Some("  ")).is_ok());
    }
}
