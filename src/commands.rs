...
pub const COMMANDS: &[Spec] = &[
    Spec {
        name: "acp",
        summary: "speak the Agent Client Protocol over stdio for IDE clients",
    },
    Spec {
        name: "agent",
    ...
    Spec {
        name: "browser",
        summary: "drive the managed browser: open, snapshot, click, type, screenshot",
    },
    Spec {
        name: "canvas",
    ...
    Spec {
        name: "claws",
        summary: "read, plan and install a CLAW.md agent package",
    },
    Spec {
        name: "commands",
    ...
    Spec {
        name: "devices",
        summary: "pair devices and manage their scoped tokens",
    },
    Spec {
        name: "directory",
    ...
    Spec {
        name: "dns",
        summary: "plan discovery records and check how a host resolves",
    },
    Spec {
        name: "docs",
    ...
    Spec {
        name: "fleet",
        summary: "create and manage isolated per-tenant cells",
    },
    Spec {
        name: "flows",
        summary: "alias for tasks",
    },
    Spec {
        name: "gateway",
    ...
    Spec {
        name: "node",
        summary: "alias for nodes",
    },
    Spec {
        name: "nodes",
        summary: "enroll, approve and inspect paired nodes and their capabilities",
    },
    Spec {
        name: "oauth",
    ...
    Spec {
        name: "pairing",
        summary: "list, approve and deny chat pairing requests",
    },
    Spec {
        name: "path",
        summary: "read one addressable leaf out of a workspace file with px://",
    },
    Spec {
        name: "plugins",
        summary: "list mcp servers, hooks and skills, and report broken ones",
    },
    Spec {
        name: "policy",
        summary: "check the setup against a written policy and print an attestation",
    },
    Spec {
        name: "promos",
        summary: "list promotional model offers and show how to claim one",
    },
    Spec {
        name: "proxy",
    ...
    Spec {
        name: "qr",
        summary: "print a scannable QR code for pairing, the dashboard, or any text",
    },
    Spec {
        name: "reset",
    ...
    Spec {
        name: "sandbox",
        summary: "inspect the container sandbox policy for shell commands",
    },
    Spec {
        name: "schema",
    ...
    Spec {
        name: "secrets",
        summary: "encrypted secret store",
    },
    Spec {
        name: "security",
    ...
    Spec {
        name: "usage",
        summary: "show proactive provider account limits and reset windows",
    },
    Spec {
        name: "webhooks",
    ...
    Spec {
        name: "wiki",
        summary: "keep a linked markdown vault: read, write, search, lint",
    },
    Spec {
        name: "workboard",
        summary: "alias for board",
    },
    Spec {
        name: "worktrees",
...
pub const NOT_BUILT: &[(&str, &str)] = &[
    (
        "clawbot",
        "a legacy alias namespace; `phoenix qr` is the command it wrapped",
    ),
    (
        "crestodian",
        "a legacy repair helper; `phoenix doctor` covers repair here",
    ),
    (
        "voicecall",
        "needs a telephony provider; phoenix speaks text and voice notes, not phone calls",
    ),
    (
        "worker",
        "a legacy cloud worker runtime; `phoenix tasks` runs background work here",
    ),
];
...
pub fn list_text(json: bool) -> String {
    ...
    out.push_str(&format!(
        "\n{} reference commands are not built here (reasons: phoenix commands --json):\n",
        NOT_BUILT.len()
    ));
...
pub fn capability_text(cfg: &Config) -> String {
    ...
    let capabilities = crate::providers::capabilities(cfg);
    ...
    out.push_str(&format!(
        "  usage         {:?}\n",
        capabilities.response_usage
    ));
    out.push_str(&format!("  quota         {:?}\n", capabilities.quota));
    let has_key = crate::providers::has_credential(cfg);
    out.push_str(&format!(
        "  credential    {}\n",
...
mod tests {
    ...
    fn capability_reports_the_wire_format_and_context() {
        ...
        let overridden = capability_text(&cfg);
        assert!(
            overridden.contains("wire format   openai-responses"),
            "{overridden}"
        );
    }
}
