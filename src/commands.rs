...
pub const NOT_BUILT: &[(&str, &str)] = &[
    (
        "clawbot",
        "an OpenClaw legacy alias namespace; `phoenix qr` is the command it wrapped",
    ),
    (
        "crestodian",
        "an OpenClaw-specific repair helper; `phoenix doctor` covers repair here",
    ),
    (
        "openclaw",
        "the OpenClaw root command; here the root command is `phoenix`",
    ),
    (
        "voicecall",
    ...
    (
        "worker",
        "an OpenClaw cloud worker runtime; `phoenix tasks` runs background work here",
    ),
];
