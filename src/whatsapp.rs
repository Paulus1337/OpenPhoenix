...
impl WhatsApp {
    ...
    fn messages_url(&self) -> String {
        format!("{GRAPH}/{}/messages", self.phone_id)
    }

    fn post_message(&self, payload: &Value) -> Result<(), String> {
        ureq::post(&self.messages_url())
    ...
    pub fn typing_payload(message_id: &str) -> Value {
        json!({
            "messaging_product": "whatsapp",
            "status": "read",
    ...
    fn typing(&self, message_id: &str) {
        let _ = self.post_message(&Self::typing_payload(message_id));
    }

    pub fn working(&self, message_id: &str) -> crate::working::Working {
        let message_id = message_id.to_string();
        let whatsapp = self.clone();
        crate::working::Working::native(
            crate::working::WHATSAPP_INTERVAL,
            move || whatsapp.typing(&message_id),
            || {},
        )
    }
    ...
    pub fn serve(&self, listener: TcpListener, handler: &mut dyn FnMut(&str, &str) -> String) {
        ...
        for stream in listener.incoming() {
            ...
            if let Err(e) = self.handle(stream, handler, &mut last_seen) {
                ...
                crate::log::warn_with(
                    "whatsapp",
                    format!("webhook request failed: {msg}"),
    ...
    fn handle(
        ...
        match method.as_str() {
            ...
            "POST" => {
                ...
                for (from, label, message_id, text) in parse_events(&v) {
                    ...
                    let mut working = self.working(&message_id);
...
pub fn parse_events(v: &Value) -> Vec<(String, String, String, String)> {
    let mut stamped: Vec<(u64, String, String, String, String)> = Vec::new();
    let empty = Vec::new();
    for entry in v["entry"].as_array().unwrap_or(&empty) {
        for change in entry["changes"].as_array().unwrap_or(&empty) {
            ...
            for msg in change["value"]["messages"].as_array().unwrap_or(&empty) {
                ...
                if from.is_empty() || message_id.is_empty() || body.is_empty() {
                    continue;
                }
                ...
                let label = contacts
                    ...
                    .to_string();
                stamped.push((ts, from, label, message_id, body));
            }
        }
    }
    ...
    stamped
        .into_iter()
        .map(|(_, from, label, message_id, body)| (from, label, message_id, body))
        .collect()
}
...
mod tests {
    ...
    #[test]
    fn typing_payload_uses_the_cloud_api_message_id_shape() {
        assert_eq!(
    ...
    fn event_parsing() {
        let payload = serde_json::json!({
            "entry": [{"changes": [{"value": {"messages": [
                {"type": "text", "from": "4915551234567", "id": "wamid.1",
                 "text": {"body": "hello phoenix"}},
                {"type": "image", "from": "4915551234567"},
                {"type": "text", "from": "", "id": "wamid.bad", "text": {"body": "x"}}
            ]}}]}]
        });
    ...
    fn events_come_back_in_message_timestamp_order() {
        let payload = serde_json::json!({
            "entry": [{"changes": [{"value": {"messages": [
                {"type": "text", "from": "4915551234567",
                 "id": "wamid.2", "timestamp": "1722500002", "text": {"body": "second"}},
                {"type": "text", "from": "4915551234567",
                 "id": "wamid.1", "timestamp": "1722500001", "text": {"body": "first"}}
            ]}}]}]
        });
        let events = parse_events(&payload);
        ...
        assert_eq!(events[1].3, "second");

        let unstamped = serde_json::json!({
            "entry": [{"changes": [{"value": {"messages": [
                {"type": "text", "from": "1", "id": "a", "text": {"body": "a"}},
                {"type": "text", "from": "1", "id": "b", "text": {"body": "b"}}
            ]}}]}]
        });
        let events = parse_events(&unstamped);
        assert_eq!(
            (events[0].3.as_str(), events[1].3.as_str()),
            ("a", "b"),
