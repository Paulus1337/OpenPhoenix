...
pub fn gather(
        ...
        if let Ok(raw) = std::fs::read_to_string(src) {
            if let Ok(v) = serde_json::from_str::<serde_json::Value>(&raw) {
                ...
                if !primary.is_empty() {
                    ...
                    if !p.is_empty() {
                        specs.push((p, m, "old nest primary".into()));
                    }
                }
                if let Some(a) = v["agents"]["defaults"]["model"]["fallbacks"].as_array() {
                    for f in a.iter().filter_map(|x| x.as_str()) {
                        ...
                        if !p.is_empty() {
                            specs.push((p, m, "old nest fallback".into()));
                        }
                    }
                }
                notes.push(format!("openclaw config: {}", src.display()));
                gateway = Some(v);
            }
        }
    }
    ...
    for (kind, ..) in onboard::PROVIDERS.iter() {
        ...
        if let Some(m) = default_model_for(kind) {
            specs.push((kind.to_string(), m.to_string(), "detected key".into()));
        }
    }
    for (p, _) in keys.iter() {
        if let Some(m) = default_model_for(p) {
            specs.push((p.clone(), m.to_string(), "carried key".into()));
        }
    }
    if ollama_up {
...
pub fn ask_migrate(
    ...
    let _ = writeln!(w, "\nFound an existing OpenClaw setup at {}", src.display());
    ...
    let _ = writeln!(
        ...
        "{}",
        if yes {
            "migrating your OpenClaw setup."
        } else {
            "starting fresh; OpenClaw is left untouched."
        }
    );
    yes
}
...
pub fn auto_first_run(
    ...
    if telegram_carried && crate::service::systemd_available() {
        if other_gateway_running() {
            let _ = writeln!(
                w,
                "beacon on hold: another gateway (openclaw) is running and would \
fight for the bot. When ready: openclaw gateway stop && phoenix service install"
            );
        } else {
...
mod tests {
    ...
    #[test]
    fn migration_prompt_honours_no_and_leaves_openclaw_alone() {
        let d = tmp("ask-no");
        let src = d.join("openclaw.json");
        fs::write(&src, "{}").unwrap();
        let found = FoundNest {
